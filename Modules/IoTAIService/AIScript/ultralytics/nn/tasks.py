...
import cnclip
from cnclip import load_from_name, available_models
import torch
...
class WorldModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        if cache_clip_model:
            ...
            model.eval()
            self.preprocess=preprocess
        text_token = cnclip.tokenize(text).to(device)
        with torch.no_grad():
            # 文本预处理：使用CNCLIP的processor对文本进行编码
            txt_feats = [self.get_encode_text(model,token) for token in text_token.split(batch)]

...
class WorldTrainer(DetectionTrainer):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides: dict[str, Any] | None = None, _callbacks=None):
        ...
        self.cnclip_model_name = "ViT-B-16" 
        # 1. 初始化时先不加载CNCLIP模型，延迟到生成嵌入时再加载
        self.cnclip_model = None
        self.cnclip_processor = None
    def _load_cnclip_model(self):
        """延迟加载CNCLIP模型，用完即释放"""
    ...
    def get_encode_text(self, model,token):
    ...
    def generate_text_embeddings(self, texts: list[str], batch: int, cache_dir: Path) -> dict[str, torch.Tensor]:
        ...
        """
        model = f"cnclip:{self.cnclip_model_name}"
        ...
        self._load_cnclip_model()
        with torch.no_grad():  # 推理阶段禁用梯度计算，节省显存
            # 文本预处理：使用CNCLIP的processor对文本进行编码
            text_token = cnclip.tokenize(texts).to(self.device)
            # 生成CNCLIP文本特征
            text_features =[self.get_encode_text(self.cnclip_model,token) for token in text_token.split(batch)]
            text_features = text_features[0] if len(text_features) == 1 else torch.cat(text_features, dim=0)
    
        # 构建文本-嵌入映射，保留原有格式
        txt_map = dict(zip(texts, text_features))
        torch.save(txt_map, cache_path)
            
        return txt_map

    def release_cnclip_memory(self):
        """主动释放CNCLIP模型占用的显存"""
        if self.cnclip_model is not None:
            LOGGER.info("Releasing CNCLIP model memory...")
            # 将模型移到CPU
            self.cnclip_model = self.cnclip_model.cpu()
            # 删除模型引用
            del self.cnclip_model
            del self.cnclip_processor
            self.cnclip_model = None
            self.cnclip_processor = None
            # 清空CUDA缓存
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            LOGGER.info("CNCLIP model memory released successfully")

...
namespace IoTAIService.AICode
{
    public class YoloLargeWorldDetectRunner
    {
        ...
        public List<YoloWorldDetectionResult> Predict(Image<Rgb24> image, float confidenceThreshold, float iouThreshold, DenseTensor<float> classEmbeds, List<string> classes)
        {
        ...
        /// <returns></returns>
        private List<YoloWorldDetectionResult> PostprocessOutput(Tensor<float> output, float confidenceThreshold, float iouThreshold, int originalWidth, int originalHeight, float gain, List<string> classes)
        {
            var results = new List<YoloWorldDetectionResult>();
            ...
            for (int i = 0; i < numBoxes; i++)
            {
                ...
                results.Add(new YoloWorldDetectionResult
                {
        ...
        private List<YoloWorldDetectionResult> ApplyNMS(List<YoloWorldDetectionResult> detections, float iouThreshold)
        {
            var finalDetections = new List<YoloWorldDetectionResult>();
            // 按类别分组处理
        ...
        /// 计算两个检测框的IOU（交并比）
        /// </summary>
        private float CalculateIOU(YoloWorldDetectionResult a, YoloWorldDetectionResult b)
        {

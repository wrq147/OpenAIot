...
namespace IoTRulesService.Flow.Builder
{
    ...
    /// 节点json转换
    /// </summary>
    public class StepJsonConverter : JsonConverter<RuleflowStep>
    {
        /// <summary>
        /// 反序列化：根据Type字段创建对应子类实例并填充数据
        /// </summary>
        public override RuleflowStep Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            // 加载JSON文档，保留完整的步骤数据
            ...
            // 转换为WorkflowStep并返回
            return stepInstance as RuleflowStep ??
                   throw new JsonException($"类型{typeName}转换为RuleflowStep失败");
        }

        /// <summary>
        /// 序列化：按实例的实际子类类型输出完整JSON
        /// </summary>
        public override void Write(Utf8JsonWriter writer, RuleflowStep value, JsonSerializerOptions options)
        {
            if (value == null)
            {

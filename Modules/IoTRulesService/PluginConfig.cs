...
namespace IoTRulesService
{
    public class PluginConfig : TANetCorePluginConfig
    {
        ...
        {
            _app = app;
            _runner = _app.ServiceProvider.GetService<RuleWheelRuner>();
            ...
            if (!string.IsNullOrEmpty(generalOption.Value.event_bus_conn))
            {
                TAEventDispatcher.Instance.RegisterPluginAllLoad(async (evt) =>
                {
                    ...
                    await bus.PubSub.SubscribeAsync<string>("IotRule", UpMsgExe, cfg =>
                    {
        ...
        /// <param name="tk"></param>
        /// <returns></returns>
        private async Task UpMsgExe(string msg, System.Threading.CancellationToken tk)
        {
            if (string.IsNullOrEmpty(msg))
            {
                var tmpoption = _app.ServiceProvider.GetService<IOptions<IotOption>>();
                var redis = _app.ServiceProvider.GetService<IotRedisHelper>();
                await redis.HashSetAsync("RuleExeNodes", tmpoption.Value.node_name, DateTime.Now.AddSeconds(130).ToString("o"));
                return;
            }

            var rs = System.Text.Json.JsonSerializer.Deserialize<BaseUpDeviceMessage>(msg, JsonMessageSerializerConfig.DefaultOptions);
            ITAServiceProvider provider = _app.ServiceProvider;
            _runner.PushConcurrentTask(rs.DeviceId, async () =>
            {
                await AnalyseDeviceUp(rs, provider).ConfigureAwait(false);
            });
        }
        ...
        /// <returns></returns>
        private async Task ExecuteRules(BaseUpDeviceMessage rs, TslReturn model, DateTime nowTime, ITAServiceProvider provider, IotRedisHelper redis)
        {
            ...
            var ruleCahce = provider.GetService<RuleCache>();
            ...
            foreach (var rule in ruleList)
            {
                ...
                var executor = await provider.GetService<RuleExecutorBuilder>().Build(rule.RuleJson);
        ...
        /// <returns></returns>
        private async Task ExcuteNotice(ITAServiceProvider provider, BaseEvent evt, MZ_IotDevice device, List<WarningNoticeItem> notices, List<Out_WarnTargetId> warnids, string extInfo)
        {
            long targetOrgId = 0;
            bool useNo = false;
                ...
                switch (noticeItem.TargetType)
                {
                    ...
                    case 3:
                        ...
                        else
                        {
                            ...
                            {
                                var tnolist = await provider.GetService<OrgExtDAL>().SelectList(x => x.OrgId == targetOrgId && x.ExtField == "IsNoRecv");
                                if (tnolist.Count > 0)
                                {
        ...
        private async Task AnalyseDeviceUp(BaseUpDeviceMessage rs, ITAServiceProvider provider)
        {
            try
            {
                ...
                var redis = provider.GetService<IotRedisHelper>();
                switch (rs.MsgType)
                {
                    case "Online":
                        {
                            ...
                            if (devicelist.Count > 0)
                            {
                                ...
                                if (!string.IsNullOrEmpty(product.StorageConfig))
                                {
                                    ...
                                    {
                                        await provider.GetService<IotInfluxBLL>().SaveOnline(rs.ProductId, rs.DeviceId, devicelist[0].Id, storageConfig, nowTime);
                                    }
                                }
                                ...
                                //获取物模型
                                model = await TslCache.GetTslModel(product.Id, redis, provider);
                                ...
                                try
                                {
                                    if (model.Model.tags != null)
                                    {
                                        ...
                                        if (needTags.Count > 0)
                                        {
                                            var initTags = await provider.GetService<IotDeviceBLL>().SelectTagsDict(devicelist[0].Id, model.Model, needTags);
                                ...
                                if (!string.IsNullOrEmpty(rdmsg.IpAddress))
                                {
                                    ...
                                    {
                                        var tagBLL = provider.GetService<IotTagBLL>();
                                        var codeBLL = provider.GetService<CodeBLL>();
                                ...
                                if (string.IsNullOrEmpty(rdmsg.RedirectFromProductId))
                                {
                                    ...
                                    {
                                        var updateDAL = provider.GetService<IotUpdateDAL>();
                                ...
                                else
                                {
                                    if (!string.IsNullOrEmpty(rdmsg.RedirecDtuId))
                                    {
                                        ...
                                        {
                                            tmpset = JsonConvert.DeserializeObject<HashSet<string>>(tids);
                                        }
                                        tmpset.Add(rdmsg.DeviceId);
                                        await redis.HashSetAsync<string>(tsourcekey, "$targetDtuIds", JsonConvert.SerializeObject(tmpset));
                                    }
                    ...
                    case "Offline":
                        {
                            ...
                            if (devicelist.Count > 0)
                            {
                                ...
                                if (storageConfig != null && storageConfig.enable == "1")
                                {
                                    await provider.GetService<IotInfluxBLL>().SaveOffline(rs.ProductId, rs.DeviceId, devicelist[0].Id, storageConfig, nowTime);
                                }

                                //向转发设备发送离线
                                if (!string.IsNullOrEmpty(tids))
                                {
                                    HashSet<string> tmpset = JsonConvert.DeserializeObject<HashSet<string>>(tids);
                    ...
                    case "ReadPropertyReply":
                        {
                            ...
                            //根据物模型转换设备属性
                            model = await TslCache.GetTslModel(rs.ProductId, redis, provider);
                            var deviceCahce = provider.GetService<DeviceCache>();
                            //获取所有旧属性数据
                            ...
                            if (allDict == null)
                            {
                                if (rdmsg.IsTagSync)
                                {
                                    ...
                                    if (storageConfig != null && storageConfig.enable == "1")
                                    {
                                        ...
                                        if (devicelist.Count > 0)
                                        {
                                            await provider.GetService<IotInfluxBLL>().SaveHistory(rdmsg.ProductId, rdmsg.DeviceId, devicelist[0].Id, storageConfig, nowTime, saveDict, model.Model.properties);
                                        }
                                ...
                                else
                                {
                                    ...
                                    {
                                        //转发数据时，设备离线则发送上线报文
                                        await provider.GetService<ServerBusProxy>().SendConnect(rdmsg.ProductId, rdmsg.DeviceId, "", rdmsg.RedirectFromProductId, rdmsg.RuleIds, rdmsg.RedirecDtuId);
                                    }
                                }
                                return;
                            }
                            ...
                            if (allnneList != null)
                            {
                                ...
                                foreach (var allnne in loopvalues)
                                {
                                    ...
                                    if (allnne.needs == null || allnne.needs.Count == 0)
                                    {
                                        ...
                                        await provider.GetService<ServerBusProxy>().ConfirmPropertyReply(readmm);
                            ...
                            if (showChangeDict.Count > 0)
                            {
                                ...
                                {
                                    saveProps.Add(kvp.Key, JsonConvert.SerializeObject(kvp.Value));
                                ...
                                if (noticeProps.Count > 0)
                                {
                                    ...
                                    await serverBus.NoticeChange(rs.DeviceId, Newtonsoft.Json.JsonConvert.SerializeObject(waitdliis, new JsonSerializerSettings()
                                    {
                                ...
                                if (storageConfig != null && storageConfig.enable == "1")
                                {
                                    var influxrs = await provider.GetService<IotInfluxBLL>().SaveHistory(rdmsg.ProductId, rdmsg.DeviceId, id, storageConfig, nowTime, showChangeDict, model.Model.properties);
                            ...
                            //本地缓存
                            provider.GetService<DeviceCache>().SetDevice(rdmsg.DeviceId, allDict);
                        }
                        break;
                    case "Event":
                        {
                            ...
                            if (model == null)
                            {
                                await provider.GetService<ServerBusProxy>().Print(rs.DeviceId, "设备事件异常", "找不到对应的物模型");
                                return;
                            }
                            var evtinfo = model.Model.events.Where(x => x.code == rdmsg.EventId).FirstOrDefault();
                            if (evtinfo == null)
                            {
                                await provider.GetService<ServerBusProxy>().Print(rs.DeviceId, "设备事件异常", "找不到对应的事件标识符");
                                return;
                            }

                            //生成事件描述信息
                            var allDict = await provider.GetService<DeviceCache>().GetDevice(rdmsg.DeviceId);
                            ...
                            //获取设备信息
                            var deviceList = await provider.GetService<IotDeviceDAL>().SelectList(x => x.DeviceId == rdmsg.DeviceId);
                            if (evtinfo.Level >= 0)
                            {
                                var product = await provider.GetService<IotProductDAL>().Select(rs.ProductId);
                                //根据事件的沉默周期限制报警频率
                                ...
                                {
                                    //添加告警
                                    var warnlist = await provider.GetService<IotWarningBLL>().Insert(evtinfo, rdmsg, device, extInfo);
                                    //添加通知
                                    if (product != null && !string.IsNullOrEmpty(product.NoticeWay))
                                    {
                                        ...
                                        await ExcuteNotice(provider, evtinfo, device, noticeList, warnlist, extInfo);
                                    }
                                }
                            }
                            foreach (var device in deviceList)
                    ...
                    case "QueryICCIDReply":
                        {
                            ...
                            var cardBLL = provider.GetService<IotCardBLL>();
                            var deviceDAL = provider.GetService<IotDeviceDAL>();
                            var devicelist = await deviceDAL.SelectList(x => x.DeviceId == rs.DeviceId);
                            if (devicelist.Count > 0)
                            {
                                ...
                                if (!tmprsp.IsSuccess())
                                {
                                    await provider.GetService<ServerBusProxy>().Print(rs.DeviceId, "物联网卡异常", tmprsp.Message);
                                }
                            }
                        }
                        return;
                    case "ChangeProduct":
                        {
                            ...
                            await provider.GetService<IotDeviceBLL>().ChangeProductId(rdmsg.DeviceId, rdmsg.TargetProductId);
                        }
                        return;
                    case "StartReadAll":
                        {
                            ...
                            var cahceDev = provider.GetService<DeviceCache>();
                ...
                //异步执行规则
                await ExecuteRules(rs, model, nowTime, provider, redis);
            }
            catch (Exception ex)
            {
                if (rs != null && !string.IsNullOrEmpty(rs.DeviceId))
                {
                    await provider.GetService<ServerBusProxy>().Print(rs.DeviceId, "设备消息异常", ex.Message);
                }
                else
                {

<template>
	<view id="PoolDetails">
		<top leftIcon="icon-fanhui" :isleftBack="true" backgroundColor="#ffffff" title="详情"
		...
		<view class="PoolDetails-content">
			...
			<view class="Avatar-information-text">
				<view class="Avatar-information-text-item" v-if="arr.LeaderName">
					<view class="Avatar-information-text-item-title">负责人</view>
			...
			<view v-if="select==2">
				<view class="Follow-up-input" @click="handComment">
					<text class="iconfont icon-tianxiegenjinhepinglun"></text>
					<input class="searchInput" type="text" placeholder="请输入评论" placeholder-style="color:#C1C1C1;">
				</view>
		...
		<view class="AddPopUps" v-if="alertHide">
			...
			<view v-if="alertName=='comment'">
				...
				<view>
					<textarea v-model="content" placeholder="请输入评论" class="addPool-textarea"
						:rows="4" />
		...
		handAddFloow() {
			...
			handDelete() {
				...
				this.$refs.promptMsg.noticeOpen(
					"你确定要删除名字为 " + this.arr.CompanyName + "线索吗?"
				)
			},
			handConfirm() {
				if (this.reason == '') {

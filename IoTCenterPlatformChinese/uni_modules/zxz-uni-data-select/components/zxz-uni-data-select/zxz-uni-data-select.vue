	...
	<view class="uni-stat__select">
		...
		<view class="uni-stat-box" :class="{'uni-stat__actived': current}">
			<view class="uni-select" :style="{height:multiple?'100%':' 35px'}"
				:class="{'uni-select--disabled':disabled,'custom_select':iscustom}">
				<view class="uni-select__input-box" :style="{height:multiple?'100%':'35px'}" @click="toggleSelector">
					<view class="" style="display: flex;flex-wrap: wrap;width: 100%;" v-if="multiple&&current.length>0">
						...
						<input @focus="selectOnfocus" v-if="filterable&&!disabled" @input.lazy="inputChange" class="uni-select__input-text"
							type="text" style="font-size: 32rpx;height: 52rpx;margin-left: 6px;width: auto;"
							placeholder="请输入" v-model="filterInput" placeholder-style="color:#C1C1C1">
					</view>
					<view v-else-if="current&&current.length>0&&!showSelector" class="uni-select__input-text">
						{{current}}
					</view>
					<input @focus="selectOnfocus" v-else-if="filterable&&showSelector" :focus="isFocus" @input.lazy="inputChange"
						:disabled="disabled" @click.stop="" class="uni-select__input-text" type="text"
						style="font-size: 32rpx;position: absolute;z-index: 1;" :placeholder="placeholderOld"
						v-model="filterInput" placeholder-style="color:#C1C1C1">
				...
				<view class="uni-select__selector" v-if="showSelector">
					...
					<scroll-view scroll-y="true" class="uni-select__selector-scroll">
						...
						<view v-else :class="['uni-select__selector-item', {'uni-select_selector-item_active' : currentArr.includes(item[dataValue])}]"
	...
	export default {
		...
		methods: {
			...
			inputChange(e) {
				if(this.isCanCustom){
					if (this.filterable && this.filterInput) {
						let filList= this.mixinDatacomResData.filter(e => e[this.dataKey].includes(this.filterInput))
						if(!filList||filList==undefined||filList.length==0){
							this.mixinDatacomResData=this.localdata
							let obj={}
							obj[this.dataKey]=e.detail.value
							obj[this.dataValue]=e.detail.value
							this.mixinDatacomResData.push(obj)
							this.$forceUpdate()
						}
					} 
				}
...
.custom_select .uni-select__input-box{

<template>
	<view class="uni-forms-item"
		:class="['is-direction-' + localLabelPos ,border?'uni-forms-item--border':'' ,border && isFirstBorder?'is-first-border':'',isDis?'dis_form_item':'',isfirstTop?'first_top':'',isLastTop?'last_top':'',noBottomMar?'no_bottom_margin':'']">
		<slot name="label">
			<view class="uni-forms-item__label" :class="{'no-label':!label && !required}"
				:style="{width:localLabelWidth,justifyContent: localLabelAlign,'font-size':labelFont,color:labelColor,'line-height':labelFont,'padding':isDis?'0 0 30rpx':'0 0 40rpx'}" v-if="showLabel">
				<text v-if="required" class="is-required" :style="{'opacity':requireOpacity}">*</text>
				<text>{{label}}</text>
			</view>
		</slot>
		<!-- #ifndef APP-NVUE -->
		<view class="uni-forms-item__content" :style="{'font-size':contentFont}">
			<slot></slot>
			<view class="uni-forms-item__error" :class="{'msg--active':msg}" :style="{'left':errorMsgLeft}">
				<text>{{msg}}</text>
			</view>
		</view>
		<!-- #endif -->
...
<script>
	...
	export default {
		...
		props: {
			...
			labelFont:{//label的文字大小
				type: String,
				default: '28rpx'
			},
			contentFont:{//content的文字大小
				type: String,
				default: '28rpx'
			},
			requireOpacity:{
			...
			isfirstTop:{//是否是第一个item
				type: [Boolean],
				default: false
			},
			isLastTop:{//是否是最后一个item
				type: [Boolean],
				default: false
			},
			noBottomMar:{//是否是没有底部外边距
				type: [Boolean],
				default: false
			}
		},

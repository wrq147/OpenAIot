<template>
	<view style="width: 100%;">
		<view v-if="disabled" style="width: 100%;">
			<view class="view_input" :class="{'disabled_view':disabled}">
		...
		<uni-popup :ref="'deptSelect'+keyId" :mask-click="false" background-color="#161A26" :safe-area="true"
			...
			<view style="width: 100%;height: 100vh;position: relative;">
				<employeeSelect :ref="'empsel'+keyId" topTitle="选择部门" :multiple="multiple" type="dept"
		...
		methods: {
			...
			choiceDepartment() {
				...
				if (!this.disabled) {
					...
					// } else {
					// 	uni.navigateTo({
					// 		url: '/pages_flow/inventory/employee_select?selected=' + selected + '&type=dept'
					// 	})
					// }

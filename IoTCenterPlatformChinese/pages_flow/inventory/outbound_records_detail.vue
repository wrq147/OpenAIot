		...
		<view class="detail_con" style="background-color: rgba(245, 248, 249, 1);">
			...
			<view class="basic_info_list" :style="{'margin-top':form.List&&form.List.length>0?'20rpx':'0'}">
				...
				<view class="basic_info_li" v-for="(val,inx) in basicInfoObj" v-if="val.Value">
					...
					<view class="li_val" v-if="val.Key!='Tabulator'&&val.Key!='Warehousing method'">{{val.Value}}</view>
					...
					<view class="url_jump" v-if="val.Key=='Logistics tracking number'" @click="jumpToLogistics">
						点击查看物流信息
					</view>
				</view>
			</view>
				...
				try {
					...
					if (row.Status == 0 || row.Status == 1) {
						...
						this.$refs.promptMsg.noticeOpen(
							'你确定要撤销编号为 ' + row.StockNumber + '的出库单吗? (此操作是不可逆的)?'
						)
					}
				} catch (e) {
					console.log(e);
			...
			async getDetailInfo() {
				...
				try {
					...
					this.basicInfoObj = [{
						...
						{
							Value: this.form.LeaveMethod==3?'':this.form.ExpressCompanyName,
							Name: '物流公司',

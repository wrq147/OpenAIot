	...
	<view style="width: 100%;">
		<view v-if="disabled" style="width: 100%;">
			<view class="image_con">
				...
				<view class="image_li no_image" v-if="!previewArr||previewArr.length==0">
					<custom-icons iconsName="icon-shangchuantupian" iconsSize="60rpx"
						iconsColor="#C6C6CA"></custom-icons>
				</view>
			</view>
		...
		<view v-else style="width: 100%;">
			<view class="image_con">
				...
				<view class="image_li no_image" @click.stop="uploadImg(previewArr,maxSize)" v-if="!disabled&&Number(maxNumber) - Number(value.length)>0">
		...
		computed: {
			...
			previewArr() {
				let arr = this.showarr.map((x) => {
					if (x && x.indexOf('http') > -1 || serverUrl.getServerUrl() == '/') {
						return x
					} else if (x) {
						return serverUrl.getServerUrl() + x
					} else {
						return ''
					}
				})

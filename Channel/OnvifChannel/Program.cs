...
namespace OnvifChannel
{
    class Program
    {
        ...
        public static IHostBuilder CreateHostBuilder(string[] args)
        {
            ...
            {
                var configSec = configuration.GetSection("OnvifOption");
                services.Configure<OnvifOption>(configSec);
                services.AddSingleton<MinioHelper>();
                services.AddEventBus(x =>
                {
                    var option = configSec.Get<OnvifOption>();
                    ...
                    config.Name = "Onvif接入";
                    config.Code = "Onvif";
                    config.Remark = "监控设备通过Onvif协议接入平台";
                ...
                services.AddHostedService<OnvifHostService>();

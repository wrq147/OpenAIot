...
{
    public static class FastBufferHelper
    {
        /// <summary>
...
#pragma warning restore 3001, 3002
        ...
        public static Int32 ReadInt32CDAB(
    byte[] buffer,
    int offset)
        {
            if (buffer.Length < offset + 4)
        ...
        public static Int32 ReadInt32BADC(
byte[] buffer,
int offset)
        {
            if (buffer.Length < offset + 4)
        ...
        /// <param name="i"></param>
        /// <returns></returns>
        public static byte[] ModbusMatch2Bytes(string mode, ModbusMatch mm, int i)
        {
            ...
            if (mm.FuncCode == 1 || mm.FuncCode == 2)
            {
            ...
            if (mode == "TCP")
            {
                ...
                wrapBytes.WriteUInt16BE(Convert.ToUInt16(i));
            ...
            else
            {
                if (mode == "RTU")
                {
                    ushort crc = CalcCRC16(writeBytes.ToArray(), 0, writeBytes.Length);
                    writeBytes.WriteUInt16LE(crc);
                }
                else
                {
                    ushort crc = CalcLRC(writeBytes.ToArray(), 0, writeBytes.Length);
                    writeBytes.WriteUInt16LE(crc);
                }
        ...
        /// <returns></returns>
        public static string ByteToHexStr(byte[] bytes, bool space = false)
        {
            string returnStr = "";
            if (bytes != null)
            {
                for (int i = 0; i < bytes.Length; i++)
                {
                    if (space)
                    {
                        returnStr = returnStr + bytes[i].ToString("X2") + " ";
                    }
                    else
                    {
                        returnStr = returnStr + bytes[i].ToString("X2");
        ...
        /// <returns></returns> 
        public static byte[] StrToToHex(string hexString)
        {

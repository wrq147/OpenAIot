            ...
            <view class="container-info" v-for="item in rangeList" :key="item.Id">
                <view class="infoBox">
                    <view class="top">
                        ...
                        <view class="icon">
                            <image v-if="getSerVerUrl()"
                                :src="getSerVerUrl()+'/appimg/treeDel.png'"
                                mode=""
                                @click="treeDelete(item)"></image>
                            <image v-if="getSerVerUrl()"
                                :src="getSerVerUrl()+'/appimg/treeEdit.png'"
                                mode=""
                                style="margin-left: 40rpx"
                                @click="treeEdit(item)"></image>
                        </view>
                    </view>
                    <view class="bottom">
                ...
                <view class="popup-container">
                    ...
                    <uni-forms
                        ...
                        <uni-forms-item label="所属分类" required name="categoryId">
                            <uni-data-select
                                v-model="valiFormData.categoryId"
                                :clear="false"
                                :localdata="range"
...
export default {
    data() {
        return {
            ...
            rules: {
                name: {
                    rules: [{ required: true, errorMessage: '车间名称不能为空' }],
                },
                categoryId: {
                    rules: [{ required: true, errorMessage: '所属分类不能为空' }],
                },
                leaderId: {
                    rules: [{ required: true, errorMessage: '请选择负责人' }],
                },
            },
            relevance: [
                { text: '是', value: 'true' },
    ...
    methods: {
        getList() {
            roomList({ TargetOrgId: this.$store.state.user.orgId }).then((res) => {
        ...
        treeEdit(item) {
            this.peopleX = [];
            roomListInfo({ id: item.Id }).then((res) => {
        ...
        treeDelete(item) {
            ...
            uni.showModal({
                ...
                success: function (res) {
                    if (res.confirm) {
                        roomListDelete({ id: item.Id }).then((res) => {
                            uni.showToast({
                                title: '删除成功',
                                icon: 'success',
                                duration: 2000,
        ...
        peopleSelect(type) {
            ...
            if (type === 1) {
                ...
                uni.navigateTo({
                    url:
                        '/pages_flow/inventory/employee_select?type=user&multiple=true&selected=' +

<template>
	<view
		style="width: 100%;height: 100vh;overflow-y: scroll;position: absolute;top: 0;line-height: 64rpx;background-color: #fff;">
		<top :title="topTitle" leftWidth="157rpx" leftIcon="icon-fanhui" :rightText="allSelect" rightWidth="157rpx"
			:isleftBack="false" @clickLeft="clickLeft" backgroundColor="#ffffff" @clickRight="handleCheckAllChange">
		</top>
		<search-compt :isOnlySearch="true" pal="请输入名称" @searching="searchUser" :zIndex="997" v-if="type=='user'"
				...
				<view class="employee_li" v-for="row in orgs" @click.stop="nextNode(row)">
					<view class="li_left">
						...
						<view class="icons_con" v-if="!setSelectIcon(row)&&row.type=='user'">
							<custom-icons iconsName="icon-weixuanzhong" iconsSize="32rpx"
								iconsColor="#EAEAEA"></custom-icons>
						</view>
						<view class="icons_con" v-if="!setSelectIcon(row)&&type=='dept'"
							@click.stop="selectChange(row)">
							<custom-icons iconsName="icon-weixuanzhong" iconsSize="32rpx"
								iconsColor="#EAEAEA"></custom-icons>
						</view>

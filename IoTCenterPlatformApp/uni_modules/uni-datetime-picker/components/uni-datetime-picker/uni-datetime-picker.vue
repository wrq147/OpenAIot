<template>
	<view class="uni-date" :class="{'dark_picker':isDark,'cus_picker':isCustom}">
		<view class="uni-date-editor" @click="show">
			<slot>
				<view class="uni-date-editor--x"
					:class="{'uni-date-editor--x__disabled': disabled,'uni-date-x--border': border}">
					<view v-if="!isRange" class="uni-date-x uni-date-single">
						<uni-icons class="icon-calendar" type="calendar" :color="isCustom&&displayValue?'#333333':'#c0c4cc'" size="22"></uni-icons>
						<view class="uni-date__x-input" :class="{'uni-date-x_hasval':isCustom&&displayValue}">{{ displayValue || singlePlaceholderText }}</view>
					</view>

					<view v-else class="uni-date-x uni-date-range">
						<uni-icons class="icon-calendar" type="calendar" :color="isCustom&&displayValue?'#333333':'#c0c4cc'" size="22"
							v-if="!isCustom"></uni-icons>
						<custom-icons class="icon-calendar" v-if="isCustom" iconsName="icon-xuanzeshijian"
							iconsSize="28rpx" iconsColor="#999999"></custom-icons>
						<view class="uni-date__x-input text-center"
							:class="{'uni-date-x_hasval':isCustom&&displayRangeValue.startDate}">
						...
						<view class="uni-date__x-input text-center"
							:class="{'uni-date-x_hasval':isCustom&&displayRangeValue.endDate}">
		...
		<view v-show="pickerVisible" class="uni-date-mask--pc" @click="close" :class="{'customClass':isCustom}"></view>
		...
		<view v-if="!isPhone" v-show="pickerVisible" ref="datePicker" class="uni-date-picker__container">
			<view v-if="!isRange" class="uni-date-single--x" :style="pickerPositionStyle">
				...
				<Calendar ref="pcSingle" :showMonth="false" :start-date="calendarRange.startDate"
					...
					:default-value="defaultValue" style="padding: 0 8px;" :isChangeBottom="isChangeBottomChange"
					:changeBottom="changeBottomChange" :isCustom="isCustom" :customMskTop="customMskTops"
					@close="iconClose" :isDark="isDark" />
			...
			<view v-else class="uni-date-range--x" :style="pickerPositionStyle">
				...
				<view v-if="hasTime" class="popup-x-header uni-date-changed">
					<view class="popup-x-header--datetime">
						...
						<time-picker type="time" v-model="tempRange.startTime" :start="timepickerStartTime"
							:border="false" :disabled="!tempRange.startDate" :hideSecond="hideSecond" :isDark="isDark">
					...
					<view class="popup-x-header--datetime">
						...
						<time-picker type="time" v-model="tempRange.endTime" :end="timepickerEndTime" :border="false"
							:disabled="!tempRange.endDate" :hideSecond="hideSecond" :isDark="isDark">
				...
				<view class="popup-x-body">
					<Calendar ref="left" :showMonth="false" :start-date="calendarRange.startDate"
						...
						:isCustom="isCustom" :isChangeBottom="isChangeBottomChange" :changeBottom="changeBottomChange"
						:customMskTop="customMskTops" @close="iconClose" :isDark="isDark" />
					<Calendar ref="right" :showMonth="false" :start-date="calendarRange.startDate"
						...
						style="padding: 0 8px;border-left: 1px solid #F1F1F1;" :isCustom="isCustom"
						:isChangeBottom="isChangeBottomChange" :changeBottom="changeBottomChange"
						:customMskTop="customMskTops" @close="iconClose" :isDark="isDark" />
				</view>
		...
		<Calendar v-if="isPhone" ref="mobile" :clearDate="false" :date="calendarDate" :defTime="mobileCalendarTime"
			...
			:hideSecond="hideSecond" @confirm="mobileChange" @maskClose="close" :isChangeBottom="isChangeBottomChange"
			:changeBottom="changeBottomChange" :isCustom="isCustom" :customMskTop="customMskTops" @close="iconClose"
			:isDark="isDark" />
	</view>
</template>
<script>
	...
	export default {
		...
		data() {
			return {
				changeBottomChange: '', //日期选择弹窗底部定位距离
				isChangeBottomChange: false, //日期选择弹窗底部定位是否发生变化
				isRange: false,
				hasTime: false,
				displayValue: '',
				inputDate: '',
		...
		watch: {
			...
			changeBottom: {
				immediate: false,
				handler(newVal) {
					this.changeBottomChange = newVal
					this.isChangeBottomChange = true
					// console.log("传的值有变化吗", newVal,this.changeBottomChange,this.isChangeBottomChange);
				}
			},
			type: {
				immediate: true,
		...
		methods: {
			...
			mobileChange(e) {
				...
				this.$emit('cusMaskShow', false) //监听自定义弹出层的显示
			},
...
<style lang="scss">
	...
	.cus_picker{
		...
		.uni-date-x{
			...
			/* white-space: nowrap; */
			overflow-y: hidden;
			.uni-date__x-input.uni-date-x_hasval {
				color: #333333;
			}
			.icon-calendar {
				padding-left: 20px;
	...
	.dark_picker .uni-date-x--border {
		border: 1rpx solid rgba(255, 255, 255, 0.20);
		border-radius: 10rpx;
	}

	.dark_picker .uni-date-x {
	...
	.dark_picker .uni-date-x .uni-date-x_hasval {
	...
	.dark_picker .uni-date-x .uni-date__x-input {
		font-size: 28rpx;
		overflow: scroll;
		white-space: nowrap;
	...
	.dark_picker .uni-date-changed--time-date {
		color: rgba(255, 255, 255, 1);
	}

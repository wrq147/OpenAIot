<template>
	<view class="uni-easyinput" :class="{ 'uni-easyinput-error': msg,'uni-easyinput_custom':isCustom }" :style="boxStyle">
		<view class="uni-easyinput__content" :class="inputContentClass" :style="inputContentStyle">
			<uni-icons v-if="prefixIcon&&!isCustom" class="content-clear-icon" :type="prefixIcon"
				:color="focusShow?prefixIconFocusColor:prefixIconColor" @click="onClickIcon('prefix')"
				:size="prefixIconSize"></uni-icons>
			<uni-icons v-if="prefixIcon&&isCustom" custom-prefix="iconfont" class="content-clear-icon" :type="prefixIcon"
				:color="focusShow?prefixIconFocusColor:prefixIconColor" @click="onClickIcon('prefix')"
				:size="prefixIconSize"></uni-icons>
			<textarea v-if="type === 'textarea'" class="uni-easyinput__content-textarea"
				...
				@confirm="onConfirm" @keyboardheightchange="onkeyboardheightchange" :style="{'font-size':contentFontSize}" spellcheck="false"></textarea>
			<input v-else :type="type === 'password' ? 'text' : type" class="uni-easyinput__content-input"
				...
				@blur="_Blur" @input="onInput" @confirm="onConfirm" @keyboardheightchange="onkeyboardheightchange" spellcheck="false"/>
...
<script>
	...
	export default {
		...
		props: {
			...
			primaryColor: { //设置主题颜色，可用于修改获取焦点后的颜色
				type: String,
				default: '#2979ff'
			},
			inputHeight: { //输入框的高度设置
				type: String,
				default: '35px'
			},
			prefixIconFocusColor: { //在获取焦点时前面的图标的颜色
				type: String,
				default: '#c0c4cc'
			},
			prefixIconColor: { //在未获取焦点时前面的图标的颜色
				type: String,
				default: '#c0c4cc'
			},
			isCustom:{//是否自定义输入框前面的图标
				type: Boolean,
				default: false
			},
			styles: {
				...
				default () {
					return {
						color: '#333',
						backgroundColor: '#fff',
						disableColor: 'rgba(22, 26, 38, 0.5)',
						borderColor: '#e5e5e5'
					};
				}
			},
		...
		computed: {
			...
			inputContentStyle() {
				const focusColor = this.focusShow ? this.primaryColor : (this.disabled?this.styles.disborderColor:this.styles.borderColor);
			...
			inputStyle() {
				...
				const inputHeight = this.inputHeight ? this.inputHeight : '35px'
				const fontSize=this.contentFontSize?this.contentFontSize:"32rpx"
				return obj2strStyle({
...
<style lang="scss">
	...
	.uni-easyinput_custom .uni-easyinput__content-textarea{
		padding-left: 20rpx;
	}
	.input-padding {
		padding-left: 10px;

		...
		<view class="page-main">
			<view class="progress-list">
				<view v-for="(item,index) in connectDeviceSteps" :key="index" class="progress-item"
					...
					<image class="item-icon icon-check" :src="getSerVerUrl()+'/appimg/check-blue.svg'" v-if="curStep > index" />
					<image class="item-icon icon-loading" :src="getSerVerUrl()+'/appimg/connecting-loading.svg'" v-else />
					{{item}}
				</view>
			</view>
			<view class="tip-txt" hover-class="checkActive" @click="jmptoErr" v-if="!isNewDevice">配网失败？点击这里查看解决方法</view>
		</view>
		<msg-prompt ref="promptMsg" @confirm="afresh" @msgClose="closenotice"></msg-prompt>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				hasGetStatus: false, //是否已经访问过设备状态了
			};
		},
		destroyed() {
			this.disConn();
				...
				try {
					...
					if (rs.data == true) {
						...
						if (this.verifycount > 40) {
							if (this.isNewDevice) {
								this.$refs.promptMsg.noticeOpen('配网失败，可能是设备连接的环境WiFi的密码错误，是否重新配网？', '系统消息',
									true)
							} else {
								this.$emit('doConfigFun', 'fail');
							}
				...
				tcp.onMessage((msg) => {
					...
					if (!this.isOnline && this.curStep < 2) {
						if (hexstr.substr(0, 2) == 'OK') {
							...
							if (this.isNewDevice) {
								this.$refs.promptMsg.noticeOpen('配网失败，可能是设备连接的环境WiFi的密码错误，是否重新配网？',
									'系统消息', true)
							} else {
								this.$emit('doConfigFun', 'fail');
							}

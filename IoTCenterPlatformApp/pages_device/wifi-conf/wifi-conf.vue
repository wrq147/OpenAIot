	...
	<view>
		...
		<view class="container" v-if="activeStep!=-3">
			...
			<do-config ref="doConnect" v-if="activeStep==3" @doConfigFun="doConfigFun" @doAfresh="doAfresh"
				:ismode="setmode" :isNewDevice="isNewDevice" :isOnlyTest="isOnlyTest"></do-config>
...
<script>
	...
	import doConfig from '@/pages_device/components/distributionNetwork/do-config.vue'
	import guide from '@/pages_device/components/distributionNetwork/guide.vue'
	import error2View from '@/pages_device/components/distributionNetwork/error2-view.vue'
	import successView from '@/pages_device/components/distributionNetwork/success-view.vue'
	import inputWifiInfo from '@/pages_device/components/distributionNetwork/input-wifi-info.vue'
	import targetWifiInfo from '@/pages_device/components/distributionNetwork/target-wifi-info'
	export default {
		components: {
			guide,
			inputWifiInfo,
		...
		onLoad: function(option) {
			...
			if (!this.isOnlyTest) {
				...
				if (updateList && updateList.length > 0 && updateList.includes(this.deviceId)) {
					...
					if (num <= 2300065&&num>2300010) {
						this.isNewDevice = false
					} else if (num > 2300065||num<=2300010) {
						this.isNewDevice = true
					}
				}
			} else {

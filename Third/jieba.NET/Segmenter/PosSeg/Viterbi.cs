...
namespace JiebaNet.Segmenter.PosSeg
{
    public class Viterbi
    {
        ...
        private static void LoadModel()
        {
            ...
            _startProbs = System.Text.Json.JsonSerializer.Deserialize<IDictionary<string, double>>(startJson);

            var transJson = File.ReadAllText(Path.GetFullPath(ConfigManager.PosProbTransFile));
            _transProbs = System.Text.Json.JsonSerializer.Deserialize<IDictionary<string, IDictionary<string, double>>>(transJson);

            var emitJson = File.ReadAllText(Path.GetFullPath(ConfigManager.PosProbEmitFile));
            _emitProbs = System.Text.Json.JsonSerializer.Deserialize<IDictionary<string, IDictionary<char, double>>>(emitJson);

            var tabJson = File.ReadAllText(Path.GetFullPath(ConfigManager.CharStateTabFile));
            _stateTab = System.Text.Json.JsonSerializer.Deserialize<IDictionary<char, List<string>>>(tabJson);
        }

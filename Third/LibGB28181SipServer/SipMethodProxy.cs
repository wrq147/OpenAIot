    ...
    public class SipMethodProxy : IDisposable
    {
        ...
        public bool GetSipDeviceInfo(SipDevice sipDevice, out ResponseStruct rs)
        {
            try
            {
                ...
                {
                    string timeoutStr = rs.Code == ErrorNumber.None ? "操作超时(" + _timeout + "ms)" : "未知错误";
                    Common.Logger.LogWarning($"[{Common.LoggerHead}]->Sip代理获取设备信息失败->{JsonHelper.ToJson(rs)}->{timeoutStr}");
                    return false;
                }
            ...
            out ResponseStruct rs)
        {
            ...
            try
            {
                ...
                Common.Logger.LogWarning($"[{Common.LoggerHead}]->Sip代理获取历史视频列表失败->{JsonHelper.ToJson(rs)}");
                return -1;
            }
            finally
            {
        ...
        public bool PtzMove(PtzCtrl ptzCtrl, out ResponseStruct rs)
        {
            try
            {
                ...
                {
                    string timeoutStr = rs.Code == ErrorNumber.None ? "操作超时(" + _timeout + "ms)" : "未知错误";
                    Common.Logger.LogWarning($"[{Common.LoggerHead}]->Sip代理PTZ控制失败->{JsonHelper.ToJson(rs)}->{timeoutStr}");
                    return false;
                }
        ...
        public bool DeInvite(SipChannel sipChannel, out ResponseStruct rs)
        {
            try
            {
                ...
                {
                    string timeoutStr = rs.Code == ErrorNumber.None ? "操作超时(" + _timeout + "ms)" : "未知错误";
                    Common.Logger.LogWarning($"[{Common.LoggerHead}]->Sip代理结束推流失败->{JsonHelper.ToJson(rs)}->{timeoutStr}");
                    return false;
                }
        ...
        public bool Invite(SipChannel sipChannel, PushMediaInfo pushMediaInfo, out ResponseStruct rs)
        {
            try
            {
                ...
                {
                    string timeoutStr = rs.Code == ErrorNumber.None ? "操作超时(" + _timeout + "ms)" : "未知错误";
                    Common.Logger.LogWarning($"[{Common.LoggerHead}]->Sip代理推流失败->{JsonHelper.ToJson(rs)}->{timeoutStr}");
                    sipChannel.PushStatus = PushStatus.IDLE;
                    return false;
                }
            ...
            catch (Exception ex)
            {
                ...
                Common.Logger.LogError($"[{Common.LoggerHead}]->Sip代理推流异常->{JsonHelper.ToJson(rs)}");
                return false;
            }
            finally
            {
        ...
        public bool InviteRecord(RecordInfo.RecItem record, PushMediaInfo pushMediaInfo, out ResponseStruct rs)
        {
            ...
            catch (Exception ex)
            {
                ...
                Common.Logger.LogError($"[{Common.LoggerHead}]->Sip代理推流异常(历史视频)->{JsonHelper.ToJson(rs)}");
                return false;
            }
            finally
            {
        ...
        public bool DeviceCatalogQuery(SipDevice sipDevice, out ResponseStruct rs)
        {
            ...
            try
            {
                ...
                {
                    string timeoutStr = rs.Code == ErrorNumber.None ? "操作超时(" + _timeout + "ms)" : "未知错误";
                    Common.Logger.LogWarning($"[{Common.LoggerHead}]->Sip代理获取设备目录失败->{JsonHelper.ToJson(rs)}->{timeoutStr}");
                    return false;
                }
                ...
                Common.Logger.LogWarning($"[{Common.LoggerHead}]->Sip代理获取设备目录失败->{JsonHelper.ToJson(rs)}");
                return false;
            }
            finally
            {

...
namespace LibGB28181SipServer
{
    ...
    public static class SipMsgProcess
    {
        ...
        {
            var obj = Common.VideoChannelRecordInfo.FindLast(x => x.TaskId.Equals(tmpRecItem.Sn));
            if (obj != null)
            {
            ...
            else
            {
                ...
                Common.VideoChannelRecordInfo.Add(record);
            }
        }
                    ...
                    case "RECORDINFO":
                        ...
                        if (recObj.RecordInfo != null)
                        {
                            ...
                            if (tmpDev != null && tmpDev.SipChannels != null && tmpDev.SipChannels.Count > 0)
                            {
                                ...
                                {
                                    Common.Logger.LogDebug(
                                        $"[{Common.LoggerHead}]->收到来自{remoteEndPoint}的录像查询结果->{recObj.DeviceId}->{recObj.ChannelId}->录像结果总数为:{recObj.TatolNum}->包体:{JsonHelper.ToJson(recObj.RecordInfo)}");
        ...
        private static async Task InviteOk(SIPResponse sipResponse, RecordInfo.RecItem record)
        {
            ...
            foreach (var obj in Common.VideoChannelRecordInfo)
            {
        ...
        {
            Common.Logger.LogDebug($"收到的回复信息:\r\n远端端点：{remoteEndPoint.ToString()}\r\n内容：{sipResponse}");
        ...
        /// 设备目录接收
        /// </summary>
        public static event CatalogReceived OnCatalogReceived = null!;
        ...
        /// 平台之间心跳接收
        /// </summary>
        public static event KeepaliveReceived OnKeepaliveReceived = null!;

        public delegate void SipDeviceReadyReceived(SipDevice sipDevice);
        /// <summary>
        /// 设备就绪通知，当设备准备好的时候触发
        /// </summary>
        public static event SipDeviceReadyReceived OnDeviceReadyReceived = null;

        public delegate void DeviceStatusReceived(SipDevice sipDevice, DeviceStatus deviceStatus);
        /// <summary>
        /// 设备状态查询接收
        /// </summary>
        public static event DeviceStatusReceived OnDeviceStatusReceived = null;

        public delegate void InviteHistroyVideoFinished(RecordInfo.RecItem record);
        /// <summary>
        /// 点播完成或结束事件
        /// </summary>
        public static event InviteHistroyVideoFinished OnInviteHistoryVideoFinished = null;

        public delegate void DeviceInfoReceived(SipDevice sipDevice, DeviceInfo deivceInfo);
        /// <summary>
        /// 设备信息查询接收
        /// </summary>
        public static event DeviceInfoReceived OnDeviceInfoReceived = null;
        ...
        /// </summary>
        public static event DeviceAuthentication OnDeviceAuthentication = null!;
        #endregion
    }
}
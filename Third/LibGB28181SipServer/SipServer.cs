using GB28181SipGate.Helper;
using LibGB28181SipServer.Enums;
using LibGB28181SipServer.Helper;
using LibGB28181SipServer.Structs;
using LibGB28181SipServer.Structs.GB28181;
using LibGB28181SipServer.Structs.GB28181.Net.SDP;
using LibGB28181SipServer.Structs.GB28181.Net.SIP;
using LibGB28181SipServer.Structs.GB28181.Sys;
using LibGB28181SipServer.Structs.GB28181.XML;
    ...
    public class SipServer
    {
            ...
            int timeout)
        {
            try
            {
                ...
                Common.Logger.LogDebug($"[{Common.LoggerHead}]->发送Sip请求->{req}");
        ...
        public void PtzMove(PtzCtrl ptzCtrl, AutoResetEvent evnt, out ResponseStruct rs, int timeout = 5000)
        {
            ...
            if (Common.SipDevices.FindLast(x => x.DeviceId.Equals(ptzCtrl.SipDevice.DeviceId)) == null) //设备是否在列表中存在
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Common.Logger.LogWarning($"[{Common.LoggerHead}]->AutoResetEvent.Set异常->{JsonHelper.ToJson(exrs)}");
                }
            ...
            out ResponseStruct rs, int timeout = 5000)
        {
            try
            {
                ...
                Common.NeedResponseRequests.TryAdd(req.Header.CallId, nrt);
                Common.Logger.LogDebug($"[{Common.LoggerHead}]->发送结束回放流请求->{req}");
            ...
            out ResponseStruct rs, int timeout = 5000)
        {
            try
            {
                ...
                Common.NeedResponseRequests.TryAdd(req.Header.CallId, nrt);
                Common.Logger.LogDebug($"[{Common.LoggerHead}]->发送终止时实流请求->{req}");
                _sipTransport.SendRequestAsync(tmpSipDevice.RemoteEndPoint, req);
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public ResponseStruct Start()
        {
            Common.Logger.LogInformation($"[{Common.LoggerHead}]->配置情况->Sip服务ID->{Common.SipServerConfig.ServerSipDeviceId}");
            Common.Logger.LogInformation($"[{Common.LoggerHead}]->配置情况->本机IP地址->{Common.SipServerConfig.SipIpAddress}");
            ...
            Common.Logger.LogInformation($"[{Common.LoggerHead}]->配置情况->启用IPV6->{Common.SipServerConfig.IpV6Enable}");
            Common.Logger.LogInformation($"[{Common.LoggerHead}]->配置情况->Sip服务端口->{Common.SipServerConfig.SipPort}");
            ...
            try
            {
                Common.Logger.LogInformation($"[{Common.LoggerHead}]->启动Sip服务.");

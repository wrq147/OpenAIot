...
using LibGB28181SipServer.System;
...
namespace LibGB28181SipServer
{
    ...
    {
        public static string BaseStartPath = Environment.CurrentDirectory;
        ...
        private static string _sipServerConfigPath = Common.ConfigPath + "SipServerConfig.json";
        ...
        public static List<VideoChannelRecordInfo> VideoChannelRecordInfo
        {
            ...
            set => _videoChannelRecordInfo = value;
        }
        private static ILogger<SipServer> _logger = null;
        public static ILogger<SipServer> Logger
        {
            get => _logger;
            set => _logger = value;
        }
        ...
        public static int ReadSipServerConfig(out ResponseStruct rs)
        {
            if (!File.Exists(_sipServerConfigPath))
            {
                ...
                if (config != null && rs.Code.Equals(ErrorNumber.None))
                {
                    ...
                    try
                    {
                        var jsonStr = JsonSerializer.Serialize(_sipServerConfig, DefaultTextJsonConfig.DefaultOptions);
                        File.WriteAllText(_sipServerConfigPath, jsonStr);
                        rs = new ResponseStruct()
                        {
                            Code = ErrorNumber.None,
                            Message = ErrorMessage.ErrorDic![ErrorNumber.None],
            ...
            else
            {
                try
                {
                    string tmpStr = File.ReadAllText(_sipServerConfigPath);
                    _sipServerConfig = JsonSerializer.Deserialize<SipServerConfig>(tmpStr, DefaultTextJsonConfig.DefaultOptions);

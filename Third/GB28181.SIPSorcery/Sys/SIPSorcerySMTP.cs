    ...
    public class SIPSorcerySMTP
    {
            ...
            else
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        logger.LogDebug("Email sent to " + toAddress);
                    }
                }
                catch (Exception excp)
                {
                    logger.LogError("Exception SendEmailAsync (To=" + toAddress + "). " + excp.Message);
                }
            }
        }
        ...
        private static void RelayMail(MailMessage email)
        {
            try
            {
                ...
                logger.LogDebug("RelayMail attempting to send " + email.Subject + " via " + m_smtpServer + ":" + smtpPort + " to " + email.To);
                ...
                smtpClient.Send(email);
                logger.LogDebug("RelayMail " + email.Subject + " relayed via " + m_smtpServer + " to " + email.To);
            }
            catch (Exception ex)
            {
                logger.LogError("Exception RelayMail. " + ex.Message);
                throw;
            }
        }
    }

    ...
    public class SIPMessageCore : ISipMessageCore
    {
        ...
        private void _cameraCache_OnItemAdded(object arg1, Camera camera)
        {
            try
            {
                ...
                logger.LogDebug("_cameraCache_OnItemAdded: [" + camera.DeviceID + "," + camera.IPAddress.ToString() + ":" + camera.Port + "] item initialized.");
            }
            catch (Exception ex)
            {
                logger.LogDebug("Exception _cameraCache_OnItemAdded: " + ex.Message);
            }
        }
        ...
        public void Start()
        {
            ...
            try
            {
                logger.LogDebug("SIPMessageCore is runing at " + LocalEP.ToString());
            ...
            {
                logger.LogError("Exception Start: " + excp.Message);
            }
        }
        ...
        public void Stop()
        {
            ...
            try
            {
                logger.LogDebug("SIP Registrar daemon stopping...");
                logger.LogDebug("Shutting down SIP Transport.");

                _transport.Shutdown();
                _transport = null;

                logger.LogDebug("sip message service stopped.");
                logger.LogDebug("SIP Registrar daemon stopped.");
            }
            catch (Exception excp)
            {
                logger.LogError("Exception Stop: " + excp.Message);
            }
        }
        ...
        public void PtzControl(PTZCommand ptzcmd, int dwSpeed, string deviceId)
        {
            ...
            {
                logger.LogError("Exception PtzControl: " + excp.Message);
            }
        }
        public void DeviceStateQuery(string deviceId)
        {
            ...
            {
                logger.LogError("Exception DeviceStateQuery: " + excp.Message);
            }
        }
        ...
        public int RecordFileQuery(string deviceId, DateTime startTime, DateTime endTime, string type)
        {
            ...
            try
            {
                logger.LogDebug("RecordFileQuery started.");
                ...
                logger.LogDebug("RecordFileQuery halted.");
            }
            catch (Exception excp)
            {
                logger.LogError("Exception RecordFileQuery: " + excp.Message);
            }
            return RecordTotal;
        }
        #endregion
        ...
        public void AddMessageResponse(SIPEndPoint localEP, SIPEndPoint remoteEP, SIPResponse response)
        {
            if (response == null)
            {
                logger.LogError("对方回复为空(response == null)");
                return;
            }
        ...
        private void CatalogHandle(SIPEndPoint localEP, SIPEndPoint remoteEP, SIPRequest request, Catalog catalog)
        {
                ...
                catalog.DeviceList.Items.FindAll(item => item != null).ForEach(catalogItem =>
                {
                    ...
                    if (devCata == DevCataType.Device)
                    {
                        if (!_nodeMonitorService.ContainsKey(catalogItem.DeviceID))
                        {
                            ...
                            logger.LogDebug("CatalogHandle: nodeMonitorService.Count=" + _nodeMonitorService.Count);
                            logger.LogDebug("CatalogHandle: nodeMonitorService.TryAdd DeviceId=" + catalogItem.DeviceID);
                        }
            ...
            {
                logger.LogWarning("CatalogHandle Exception: " + ex.Message);
            }
        }
        ...
        private void KeepaliveHandle(SIPEndPoint localEP, SIPEndPoint remoteEP, SIPRequest request, KeepAlive keepAlive)
        {
            ...
            // _subscribe = true;
            logger.LogDebug("KeepAlive:" + remoteEP.ToHost() + "=====DevID:" + keepAlive.DeviceID + "=====Status:" + keepAlive.Status + "=====SN:" + keepAlive.SN);
        }

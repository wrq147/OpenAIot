...
namespace SIPSorcery.SIP.App
{
    ...
    public class SIPNotifierClient
    {
        ...
        private const int
            RESCHEDULE_SUBSCRIBE_MARGIN =
                10; // Reschedule subsequent subscriptions with a small margin to try and ensure there is no gap.
        ...
        public void Stop()
        {
            try
            {
                ...
                {
                    logger.LogDebug(
                        $"Stopping SIP notifier user agent for user {m_authUsername} and resource URI {m_resourceURI}.");
        ...
        private async Task GotNotificationRequest(SIPChannel localSipChannel, SIPEndPoint localSIPEndPoint,
            SIPEndPoint remoteEndPoint, SIPRequest sipRequest)
        {
            ...
            {
                logger.LogDebug(
                    $"SIPNotifierClient GotNotificationRequest for {sipRequest.Method} {sipRequest.URI} {sipRequest.Header.CSeq}.");
                ...
                {
                    logger.LogWarning(
                        $"A duplicate NOTIFY request received by SIPNotifierClient for subscription Call-ID {m_subscribeCallID}.");
                }
                else
                {
        ...
        private void StartSubscription()
        {
            ...
            {
                logger.LogDebug(
                    $"SIPNotifierClient starting for {m_resourceURI} and event package {m_sipEventPackage}.");
                ...
                logger.LogWarning(
                    $"Subscription attempts to {m_resourceURI} for {m_sipEventPackage} have been halted.");
            }
            catch (Exception excp)
            {
        ...
        public void Subscribe(SIPURI subscribeURI, long expiry, SIPEventPackagesEnum sipEventPackage,
            string subscribeCallID, SIPURI contactURI)
        {
            try
            {
                ...
                {
                    logger.LogWarning(
                        $"Subscription to {subscribeURI} reached the maximum number of allowed attempts without a failure condition.");
                    m_subscribed = false;
                    SubscriptionFailed?.Invoke(subscribeURI, SIPResponseStatusCodesEnum.InternalServerError,
                ...
                else
                {
                    ...
                    SIPRequest subscribeRequest = SIPRequest.GetRequest(
                        ...
                        new SIPFromHeader(null,
                            new SIPURI(m_authUsername, m_authDomain, null, m_resourceURI.Scheme, SIPProtocolsEnum.udp),
                    ...
                    SIPNonInviteTransaction subscribeTransaction =
                        new SIPNonInviteTransaction(m_sipTransport, subscribeRequest, m_outboundProxy);
            ...
            catch (Exception excp)
            {
                ...
                SubscriptionFailed?.Invoke(m_resourceURI, SIPResponseStatusCodesEnum.InternalServerError,
        ...
        {
            var responseCode = (failureReason == SocketError.TimedOut)
                ? SIPResponseStatusCodesEnum.ServerTimeout
                : SIPResponseStatusCodesEnum.InternalServerError;
            SubscriptionFailed?.Invoke(m_resourceURI, responseCode,
                $"Subscription request to {m_resourceURI} failed with {failureReason}.");
            m_waitForSubscribeResponse.Set();
        }
            ...
            SIPEndPoint remoteEndPoint, SIPTransaction sipTransaction, SIPResponse sipResponse)
        {
            try
            {
                ...
                else if (sipResponse.Status == SIPResponseStatusCodesEnum.Forbidden)
                {
                    ...
                    SubscriptionFailed?.Invoke(m_resourceURI, sipResponse.Status,
                         ...
                         sipResponse.Status == SIPResponseStatusCodesEnum.Unauthorised)
                {
                    ...
                    {
                        SubscriptionFailed?.Invoke(sipTransaction.TransactionRequestURI, sipResponse.Status,

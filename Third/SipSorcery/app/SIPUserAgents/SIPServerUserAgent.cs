    ...
    public class SIPServerUserAgent : ISIPServerUserAgent
    {
        ...
        public bool AuthenticateCall()
        {
            ...
            try
            {
                ...
                {
                    logger.LogWarning(
                        $"Rejecting authentication required call for {m_uasTransaction.TransactionRequestFrom}, SIP account not found.");
                ...
                else
                {
                    ...
                    if (authenticationResult.Authenticated)
                    {
                        ...
                        {
                            logger.LogDebug(
                                $"New call from {remoteEndPoint} successfully authenticated by IP address.");
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            logger.LogWarning(
                                $"Call not authenticated for {m_sipAccount.SIPUsername}@{m_sipAccount.SIPDomain}, responding with {authenticationResult.ErrorResponse}.");
                        }
            ...
            try
            {
                if (m_uasTransaction.TransactionFinalResponse == null)
                {
                    SIPResponse redirectResponse =
                        SIPResponse.GetResponse(m_uasTransaction.TransactionRequest, redirectCode, null);
                ...
                if (clientHungup == false)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            m_byeTransaction.NonInviteTransactionFinalResponseReceived +=
                                ByeServerFinalResponseReceived;
            ...
            try
            {
                     ...
                     sipResponse.Status == SIPResponseStatusCodesEnum.Unauthorised) && SIPAccount != null)
                {
                    ...
                    SIPNonInviteTransaction authByeTransaction =
                        new SIPNonInviteTransaction(m_sipTransport, authRequest, null);
        ...
        {
            logger.LogDebug(
                $"SIPServerUserAgent client failed with {failureReason} in transaction state {m_uasTransaction.TransactionState}.");

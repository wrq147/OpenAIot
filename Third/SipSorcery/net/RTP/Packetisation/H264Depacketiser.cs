...
namespace SIPSorcery.Net
{
    public class H264Depacketiser
    {
        ...
        List<KeyValuePair<int, byte[]>>
            temporary_rtp_payloads =
                new List<KeyValuePair<int, byte[]>>(); // used to assemble the RTP packets that form one RTP Frame

        MemoryStream
            fragmented_nal =
                new MemoryStream(); // used to concatenate fragmented H264 NALs where NALs are splitted over RTP packets

        public virtual MemoryStream ProcessRTPPayload(byte[] rtpPayload, ushort seqNum, uint timestamp, int markbit,
            out bool isKeyFrame)
        {
        ...
        public virtual List<byte[]> ProcessRTPPayloadAsNals(byte[] rtpPayload, ushort seqNum, uint timestamp,
            int markbit, out bool isKeyFrame)
        {
        ...
        protected virtual List<byte[]> ProcessH264Payload(byte[] rtp_payload, ushort seqNum, uint rtp_timestamp,
            int rtp_marker, out bool isKeyFrame)
        {
            ...
            temporary_rtp_payloads.Add(new KeyValuePair<int, byte[]>(seqNum,
                rtp_payload)); // TODO could optimise this and go direct to Process Frame if just 1 packet in frame
            if (rtp_marker == 1)
            {
                ...
                if (temporary_rtp_payloads.Count > 1)
                {
                    temporary_rtp_payloads.Sort((a, b) =>
                    {
                        ...
                        return (Math.Abs(b.Key - a.Key) > (0xFFFF - 2000))
        ...
        protected virtual List<byte[]> ProcessH264PayloadFrame(List<KeyValuePair<int, byte[]>> rtp_payloads,
            out bool isKeyFrame)
        {
            bool? isKeyFrameNullable = null;
            List<byte[]>
                nal_units =
                    new List<byte[]>(); // Stores the NAL units for a Video Frame. May be more than one NAL unit in a video frame.
            ...
            for (int payload_index = 0; payload_index < rtp_payloads.Count; payload_index++)
            {
                ...
                else if (nal_header_type == 24)
                {
                    ...
                    try
                    {
                        ...
                        {
                            int size = (rtp_payloads[payload_index].Value[ptr] << 8) +
                                       (rtp_payloads[payload_index].Value[ptr + 1] << 0);
                            ptr = ptr + 2;
                            ...
                            nal_units.Add(
                                nal); // Add to list of NALs for this RTP frame. Start Codes like 00 00 00 01 get added later
                            ptr = ptr + size;
                        }
                    }
                    catch

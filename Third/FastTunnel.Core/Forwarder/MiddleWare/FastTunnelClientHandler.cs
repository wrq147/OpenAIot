...
namespace FastTunnel.Core.Forwarder.MiddleWare
{
    public class FastTunnelClientHandler
    {
        ...
        private async Task HandleClientOn(HttpContext context, StringValues version)
        {
        ...
        public async Task Handle(HttpContext context, Func<Task> next)
        {
            ...
            if (Version.Parse(version).Major != serverVersion.Major)
            {
                await Close(webSocket, $"客户端版本{version}与服务端版本{serverVersion}不兼容，请升级。");
                return;
            }

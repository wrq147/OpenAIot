	...
	<view>
		<view class="search">
			<view class="search_icon">
				...
				<uni-easyinput class="my-search-bar" :styles="styles" prefixIcon="search" clearSize="16"
					@input="iconClick" v-model="searchVal" confirm-type="search" @confirm="iconClick"
		...
		<view class="list" v-else>
			<view v-if="list.length>0" class="item" v-for="item in list" :key="item.Id">
				<navigator :url="'/pages/card_center/card_case_detail?id='+item.Id+'&uid='+uid+'&cardId='+cardId">
					...
					<view class="right">
						...
						<view class="des">
							{{item.detailText==undefined?'该案例没有添加文字描述':item.detailText}}
						</view>
					</view>
				</navigator>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			clearHistory() { //清楚历史记录
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							this.historylist = [];
							uni.setStorageSync('caseSearchList' + this.userInfo.Id, []);
							this.getHistoryList();
							this.$forceUpdate()
						}
					}
			...
			getHistoryList() {
				this.historyList = uni.getStorageSync('caseSearchList' + this.userInfo.Id) || [];
			...
			iconClick() { //点击图标或者失去焦点的时候执行
				...
				if (this.historyList.indexOf(this.searchVal) == -1 && this.searchVal != null && this.searchVal != '') {
					...
					uni.setStorageSync('caseSearchList' + this.userInfo.Id, this.historyList);
			...
			async load_data() {
				...
				// console.log("访问案例列表传递的参数", data);
				let rsp = await getExampleList(data);
				// console.log("案例列表数据", rsp);
					...
					if (!this.hashitems[it.Id]) {
						if (it.Detail != null || it.Detail != '') {
							...
							parseDetail.forEach(val => {
								...
								if (val.type == 'text') {
									detailObj = {
										detailText: val.data //将案例详情从json字符串转化为数组，然后将文字描述取出来
									}
								}
						...
						this.list.push(it); //注意this.caseductList是数组，it是对象
					}
				});
				// console.log("最后查询到的案例列表", this.caseductList);

		...
		<view class="wrap">
			<view class="top">
				...
				<view class="photo" @click="openUpload">
					<view class="img" :key="imgKey">
						<template>
							<image v-if="exmImg" class="exmImg" @click.stop="preViewImg" :src="exmImg" mode="aspectFill">
							</image>
			...
			<view class="title">
				...
				<template v-if="exmIntro!=null&&exmIntro!=''">
					<view class="inp">
						<view @click="addExmIntro" class="txt"><text style="color: red;">*</text>案例内容<uni-icons type="forward" size="20"
				...
				<template v-else>
					...
					<view class="inp">
						<view class="jswrap">
							<view class="border-all" @click="addExmIntro">
								<view class="uptip">
	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				if (this.exmTitle != "" && this.exmImg != "" && this.exmIntro != null && this.orgId != null) {
					let data = {
						orgId: this.orgId,
						title: this.exmTitle,

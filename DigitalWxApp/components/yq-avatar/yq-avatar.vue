<template name="yq-avatar">
	<view>
		...
		<view @mouseout="fEnd" @mouseenter="fStart" @t @mousedown="fStart" @mousemove="fMove" @mouseup="fEnd"
			@touchstart="fStart" @touchmove="fMove" @touchend="fEnd" class="oper-canvas"
			:style="{top: sT, height: csH}">
			<canvas canvas-id="oper-canvas" id="oper-canvas" :style="{top: sT, height: csH,width:'100%'}"
				disable-scroll="false"></canvas>
		</view>
		...
		methods: {
			...
			fMove(e) {
				...
				if (touch1) {
					...
					if (touch1.clientX !== touch0.clientX && this.letRotate) {

...
!-->
<template>
  <RoyModal v-if="visibleIn" :show.sync="visibleIn" :title="directExport ? '打印导出' : '打印预览'" class="rptd-viewer"
    height="90%" width="90%" @close="closePrint">
...
import { AutoRender } from '@/components/print/Viewer/auto-render'
...
export default {
  ...
  methods: {
    ...
    async btnClickPrint() {
      ...
      var printEle = document.getElementById("roy-viewer");
          try {
            let LODOP = ldop.getLodop()//调用getLodop获取LODOP对象
            LODOP.PRINT_INIT("")
            LODOP.ADD_PRINT_HTM(0, 0, "100%", "100%", printEle.innerHTML);
            LODOP.PREVIEW()
          } catch (err) {
            this.$alert('<a href="http://www.lodop/download.html" style="color:#0000ff;text-decoration:underline;" target="_blank">请点击此处链接</a>，下载安装插件后重试。', "提示", {

export function InitProgramNodes(graph) {
    var LiteGraph = graph;
    ...
    DebugOut.title = "模拟输出";
    DebugOut.desc = "在当前节点显示输入的数据";
    DebugOut.prototype.onExecute = function () {
        if (this.inputs[0]) {
            this.boxcolor = "#AEA";
    ...
    DebugOut.prototype.getTitle = function () {
        if (this.flags.collapsed) {
            return this.inputs[0].label;
        }
        return this.title;
    };
    DebugOut.toString = function (o) {
        if (o == null) {
            return "空";
            ...
            for (var i = 0; i < o.length; ++i) {
                str += DebugOut.toString(o[i]) + (i + 1 != o.length ? "," : "");
            }
            str += "]";
            return str;
        } else {
    ...
    DebugOut.prototype.onDrawBackground = function (ctx) {
        this.inputs[0].label = DebugOut.toString(this.value);
    };

    LiteGraph.registerNodeType("程序/模拟输出", DebugOut);
    ...
    RuningStep.title = "运行步数";
    RuningStep.desc = "返回当前运行中的步数";
    RuningStep.prototype.onExecute = function () {
        this.boxcolor = "#AEA";
        this.setOutputData(0, this.runingStep);
    };
    RuningStep.prototype.onGetInputs = function () {
        return [["触发器", LiteGraph.ACTION]];
    };
    LiteGraph.registerNodeType("程序/运行步数", RuningStep);
    ...
    Subgraph.prototype.exefun = function () {
        ...
        //execute
        this.subgraph.runStep(this.properties.循环);
        var iserr = false;
        for (var i = 0; i < this.subgraph._nodes.length; ++i) {
    ...
    Subgraph.prototype.onExecute = function () {
        ...
        if (this.properties.延时 > 0) {
            setTimeout(this.exefun, this.properties.延时);
        }
        else {
            this.exefun();
        }
    ...
    LiteGraph.registerNodeType("程序/子程序", Subgraph);
    ...
    GraphInput.title = "程序输入";
    GraphInput.desc = "子程序通过连接这个节点获取输入数据";
    ...
    LiteGraph.registerNodeType("程序/输入", GraphInput);
    ...
    GraphOutput.title = "程序输出";
    GraphOutput.desc = "子程序通过连接这个节点输出数据";
    ...
    LiteGraph.registerNodeType("程序/输出", GraphOutput);

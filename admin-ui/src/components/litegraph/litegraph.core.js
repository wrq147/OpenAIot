...
(function(global) {
    ...
    var LiteGraph = (global.LiteGraph = {
        ...
        EVENT: "action", //for outputs
        ACTION: "action", //for inputs
        ...
        use_deferred_actions: false, //executes actions during the graph execution flow
        ...
        */
        ...
        wrapFunctionAsNode: function(
            ...
            classobj.prototype.onGetInputs = function () {
    ...
    LGraph.prototype.stop = function() {
        ...
        if (this._nodes) {
            for(var i=0;i<this._nodes.length;i++){
                this._nodes[i].boxcolor=undefined;
            }
        }
        ...
        if (do_not_catch_errors) {
            ...
            for (var i = 0; i < num; i++) {
                for (var j = 0; j < limit; ++j) {
                    ...
                    node.runingStep=i;
                    node.isexe=true;
                    if(node._waiting_actions && node._waiting_actions.length)
                    {
                    ...
                    if(node.inputs){
                        for (var aii = 0; aii < node.inputs.length; aii++) {
                            if (node.inputs[aii].type==LiteGraph.ACTION) {
                                isActionNode=true;
                                break;
                            }
                        }
                    ...
                    if (node.mode == LiteGraph.ALWAYS && node.onExecute&&!isActionNode) {
                        node.doExecute();
                    }
                }
	...
	*/
        ...
        if (subnode.inputs)
            for (var i = 0; i < subnode.inputs.length; ++i) {
                ...
                if (this.drawButton(20, y + 2, w - 20, h - 2)) {
                    var type = subnode.constructor.input_node_type || "程序/输入";
                    this.graph.beforeChange();
        ...
        if (subnode.outputs)
            for (var i = 0; i < subnode.outputs.length; ++i) {
                ...
                if (this.drawButton(canvas_w - w, y + 2, w - 20, h - 2)) {
                    var type = subnode.constructor.output_node_type || "程序/输出";
                    this.graph.beforeChange();
                    var newnode = LiteGraph.createNode(type);
                    if (newnode) {
                        subgraph.add(newnode);
    ...
    LGraphCanvas.prototype.processNodeWidgets = function(
        ...
        function inner_value_change(widget, value) {
            ...
            if(that.node_panel!=null){
                //刷新节点面板
                if (that.onShowNodePanel) {
                    that.onShowNodePanel(node);
                }
                else
                {
                    that.showShowNodePanel(node);
                }
            }
    ...
    LGraphCanvas.onShowPropertyEditor = function(item, options, e, menu, node) {
        ...
        dialog.innerHTML =
            "<span class='name'></span><input autofocus type='text' class='value'/><button>确认</button>";
        ...
        function setValue(value) {
            ...
            var graphcanvas = LGraphCanvas.active_canvas;
            if(graphcanvas.node_panel!=null){
                //刷新节点面板
                if (graphcanvas.onShowNodePanel) {
    ...
    LGraphCanvas.prototype.prompt = function(title, value, callback, event, multiline) {
        ...
        if(multiline)
	        dialog.innerHTML = "<span class='name'></span> <textarea autofocus class='value'></textarea><button class='rounded'>确定</button>";
		else
        	dialog.innerHTML = "<span class='name'></span> <input autofocus type='text' class='value'/><button class='rounded'>确定</button>";
        dialog.close = function() {
    ...
    LGraphCanvas.prototype.showShowNodePanel = function( node )
	{
		...
		function inner_refresh()
		{
            ...
            // panel.addWidget( "combo", "模式", LiteGraph.NODE_MODES[node.mode], {values: LiteGraph.NODE_MODES}, fUpdate);
            ...
            panel.addWidget( "combo", "颜色", nodeCol, {values: Object.keys(LGraphCanvas.node_colors)}, fUpdate);


            panel.addWidget("number", "优先级(小到大)", node.priority,  {}, (name,value)=>{
                node.priority = value;
            });
		...
		inner_refresh();
        var optionNode=ref_window.document.querySelector("#NodeOptionsPanel");
        if(optionNode==null){
            this.canvas.parentNode.appendChild(panel);
        }
        else{
            optionNode.appendChild(panel);
        }
	...
	LGraphCanvas.prototype.showSubgraphPropertiesDialog = function(node)
	{
		...
		var panel = this.createPanel("子程序输入",{closable:true, width: 500});
		panel.node = node;
		...
		//add extra
		var html = " + <span class='label'>Name</span><input class='name'/><span class='label'>Type</span><select class='type'><option value='enum'>枚举</option><option value='number'>数值</option><option value='string'>字符串</option><option value='boolean'>布尔值</option><option value='object'>对象</option><option value='action'>事件</option></select><button>+</button>";
    ...
    LGraphCanvas.prototype.showSubgraphPropertiesDialogRight = function (node) {
        ...
        if (old_panel)
            old_panel.close();
        // new panel
        var panel = this.createPanel("子程序输出", { closable: true, width: 500 });
        panel.node = node;
        panel.classList.add("subgraph_dialog");
    ...
    LGraphCanvas.node_colors = {
        none:{color: "#333", bgcolor: "#353535", groupcolor: "#666" },
    ...
    LGraphCanvas.prototype.getCanvasMenuOptions = function() {
        ...
        if (this.getMenuOptions) {
            ...
            options = [
                ...
                { content: "添加分组", callback: LGraphCanvas.onGroupAdd },
    ...
    LGraphCanvas.prototype.getNodeMenuOptions = function(node) {
        ...
        if (node.getMenuOptions) {
            ...
            if(node.resizable !== false){
                options.push({
                    content: "调整大小", callback: LGraphCanvas.onMenuResizeNode
                });
            }
            options.push(
                {
    ...
    LGraphCanvas.prototype.getGroupMenuOptions = function(node) {
        var o = [
            { content: "修改标题", callback: LGraphCanvas.onShowPropertyEditor },
            {
                content: "修改颜色",
                has_submenu: true,
                callback: LGraphCanvas.onMenuNodeColors
            ...
            null,
            { content: "删除", callback: LGraphCanvas.onMenuNodeRemove }
        ];
     ...
     */
    function ContextMenu(values, options,names) {
        options = options || {};

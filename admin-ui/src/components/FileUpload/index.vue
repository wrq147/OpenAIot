<template>
  <div class="upload-file" :class="isShowLeft?'component-upload-flexleft':''">
    <div class="upload_con">
      ...
      <el-upload :disabled="fileList.length>0" :action="uploadFileUrl" :before-upload="handleBeforeUpload" :file-list="fileList" :limit="limit"
...
export default {
  ...
  data() {
    return {
      uploadFileUrl: process.env.VUE_APP_BASE_API == "/"
          ...
          : process.env.VUE_APP_BASE_API + "/AuthService/File/Upload?withDomain=true", // 上传的图片服务器地址
      headers: {
  ...
  methods: {
    ...
    handleDelete(index) {
      ...
      let tmparr= this.fileList.splice(index, 1);
      if(tmparr.length>0){
        console.log(tmparr[0]);
          request({
            url: '/AuthService/File/Delete',
            method: 'get',
            params: {id:tmparr[0].url}
          });
       }
      this.$emit("input", this.listToString(this.fileList));
    },

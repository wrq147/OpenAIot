<template>
    <div>
        <el-form ref="form" :model="form" label-width="80px" v-if="activeComponentParams.formKey&&activeComponentParams.formKey.length>0" :rules="rules">
                    ...
                    <el-select v-if="activeComponentParams[item].type=='remoteSelect'&&activeComponentParams[item].name=='deviceOptions'"
                     @change="onDeviceChange($event,item)" v-model="form[item]" clearable filterable remote reserve-keyword :placeholder="activeComponentParams[item].placeholder"
                      :remote-method="deviceRemoteMethod" @clear="deviceRemoteMethod('')" @remove-tag="deviceRemoteMethod('')" :loading="loading">
                        <el-option v-for="optionItem in deviceOptions" :key="optionItem[activeComponentParams[item].key]"
                         :label="optionItem[activeComponentParams[item].label]" :value="optionItem[activeComponentParams[item].value]"></el-option>
                    </el-select>

                    <el-select v-if="activeComponentParams[item].type=='select'&&activeComponentParams[item].name=='propOptions'"
                     v-model="form[item]" :placeholder="activeComponentParams[item].placeholder">
                        <el-option v-for="optionItem in propOptions" :key="optionItem[activeComponentParams[item].key]" :label="optionItem[activeComponentParams[item].label]"
                         :value="optionItem[activeComponentParams[item].value]">
                        </el-option>
                    </el-select>

                    <el-select v-if="activeComponentParams[item].type=='remoteSelect'&&activeComponentParams[item].name=='rulesOptions'"
                        v-model="form[item]" filterable remote reserve-keyword clearable
                        :placeholder="activeComponentParams[item].placeholder" :remote-method="loadRulesList" @clear="loadRulesList('')" @remove-tag="loadRulesList('')" :loading="loading">
                        <el-option v-for="optionItem in rulesOptions" :key="optionItem[activeComponentParams[item].key]" :label="optionItem[activeComponentParams[item].label]"
                            :value="optionItem[activeComponentParams[item].value]">
                        </el-option>
                    </el-select>

                    <el-select v-if="activeComponentParams[item].type=='remoteSelect'&&activeComponentParams[item].name=='planeOptions'"
                     v-model="form[item]" clearable filterable reserve-keyword :placeholder="activeComponentParams[item].placeholder" :loading="loading">
                        <el-option v-for="optionItem in planeOptions" :key="optionItem[activeComponentParams[item].key]" :label="optionItem[activeComponentParams[item].label]"
                            :value="optionItem[activeComponentParams[item].value]">
                        </el-option>
                    </el-select>

                    <el-date-picker style="width: 100%;" v-if="item=='dateRange'" v-model="dateRange" :type="activeComponentParams.dateParams.type" range-separator="至"
                     :start-placeholder="activeComponentParams.dateParams.timeText[0]"
                     :end-placeholder="activeComponentParams.dateParams.timeText[0]">
                    </el-date-picker>

                    <el-input v-if="activeComponentParams[item].type=='inputText'" class="inputText" v-model="form[item]"
                     :placeholder="activeComponentParams[item].placeholder"></el-input>
                    
                    <el-input-number v-if="activeComponentParams[item].type=='number'" v-model="form[item]"
                     :min="activeComponentParams[item].min" :step="activeComponentParams[item].step"></el-input-number>
                     ...
                     <el-radio-group v-if="activeComponentParams[item].type=='radio'" v-model="form[item]">
                        <template v-for="(it,inx) in activeComponentParams[item].list">
                            <el-radio :label="it.label" style="margin-top:5px" v-if="it.hasmargin" :key="'radio'+inx">{{it.text}}</el-radio>
...
export default {
    ...
    methods: {
        ...
        cascaderChange($event,itemParam) {
            this.form[itemParam] = $event[$event.length-1]
        },
        onImport(form) {

    ...
    submitAnalysis() {
      ...
      this.$refs["bi"].validate((valid) => {
        if (valid) {
          ...
          for (const item of this.bi.coordinate.filter.options) {
            selectPart = selectPart + this.bi.statistics.type + "(case when " + this.bi.coordinate.field + "='" + item.label + "' then " + this.bi.statistics.field + " end) as " + item.label + ",";
          }
          ...
          let executeSql = selectPart + wherePart + orderByPart;

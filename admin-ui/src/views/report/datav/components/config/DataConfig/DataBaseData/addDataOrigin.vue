    ...
    <el-dialog v-if="dialogFlag" :title="title" :visible.sync="dialogFlag" :close-on-click-modal="false" append-to-body width="700px" top="2vh" @close="cancel">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="120px" class="addPeople">
        <el-form-item label="数据库类型" prop="type">
          <el-select v-model="ruleForm.type" readonly placeholder="请选择数据库类型">
            <el-option label="MySQL" :disabled="!isBaseaName" value="mysql" />
            <el-option label="SQLServer" :disabled="!isBaseaName" value="sqlserver" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="IP地址" prop="ipAdress">
          <el-input v-model="ruleForm.ipAdress" :readonly="!isBaseaName" placeholder="请输入IP地址" />
        </el-form-item>
        ...
        <el-form-item label="密码" prop="password">
            <el-input type="password" v-model="ruleForm.password" :readonly="!isBaseaName" placeholder="请输入密码" />
        </el-form-item>
...
<script>
import { addSourse, showDatabases } from "@/api/report/sourse";
export default {
  name: 'addDataOrigin',
  props: {
    dialogVisible: {
  ...
  methods: {
    ...
    getAddSourse() {
      this.ruleForm.ipAddress = this.ruleForm.ipAdress;
      this.ruleForm.databaseType = this.ruleForm.type;
    ...
    getShowDatabases() {
      showDatabases(this.ruleForm).then(res => {

      ...
      <el-scrollbar class="right-scrollbar">
        ...
        >
          <template v-slot:staticSlot v-if="configData.chartOption.dataSourceType == 'static'">
            <el-form-item>
              <div slot="label">
                <span style="margin-right: 10px">滚动数据</span>
              </div>
              <div style="margin-top: -10px">
                <el-button type="text" @click="addItem()">+ 添加</el-button>
              </div>

              <div v-for="(tmpitem, index) in configData.chartOption.staticDataValue" :key="'a' + index" style="margin-bottom: 10px">
                <el-input placeholder="数据" size="small" v-model="configData.chartOption.staticDataValue[index]" style="width: calc(100% - 38px)"/>
                <el-button style="margin-left: 10px" size="mini" @click="delItem(index)" type="danger" icon="el-icon-delete" circle></el-button>
              </div>
            </el-form-item>
          </template>
          <template>
            <div style="margin-bottom: 20px">
              <el-table ref="multipleTable" v-loading="false" border :data="resultTableList" style="width: 100%" max-height="500" :fit="true"
                @cell-click="(row, column) => cellhandleClick({ column })" @header-click="(column) => cellhandleClick({ column })" :cell-style="cellStyle" :header-cell-style="headercellStyle">

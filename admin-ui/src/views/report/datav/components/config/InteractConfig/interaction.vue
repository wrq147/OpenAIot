<template>
  <div class="field-box">
          ...
          <el-collapse-item>
            <template slot="title">
              <div style="width: 100%;display:flex;justify-content: space-between;">
                ...
                <div>
                  ...
                  <i class="el-icon-delete active" style="margin-left:3px;margin-right:15px;font-size:16px"
                    @click.stop="delEvent"></i>
                </div>
              </div>
            </template>
            <div>
              <el-tabs @tab-click="handleClick" v-model="tabName">
                <el-tab-pane v-for="(item) in events" :key="item.key" :label="item.name"
            ...
            <div v-if="events.length > 0">
              ...
              <el-form-item label="组件配置"
                v-if="events[eventIndex].chart != undefined && events[eventIndex].act == 'change'">
    ...
    <el-dialog title="自定义条件" v-if="conditionOpen" :visible.sync="conditionOpen" width="1000px" hight="900px"
      append-to-body :close-on-click-modal="false">
      <data-editor :key="aceKey" @submitData="submitCondition" @cancelData="cancelCondition"
        :customData="events[eventIndex].condition"></data-editor>


    </el-dialog>

    <el-dialog title="自定义组件配置" v-if="chartOpen" :visible.sync="chartOpen" width="1000px" hight="900px" append-to-body
      :close-on-click-modal="false">
      <data-editor @submitData="submitChartOption" @cancelData="cancelChartOption"
        :customData="events[eventIndex].customConfig"></data-editor>
...
export default {
  ...
  methods: {
    ...
    addEvent() {
      ...
      this.events.push({
        name: "事件" + index, key: index, type: "", chartType: "", chart: null, act: "", condition: '(data) => { \n' +
          '  return true\n' +

        ...
        <el-tabs v-model="staticcurrentTab" class="center-tabs">
            <el-tab-pane label="y轴数据" name="yAxisData">
                ...
                <div style="max-height:300px;overflow-y:scroll;margin-top:10px">
                    <div v-for="(tmpitem, index) in configData.chartOption.staticDataValue[0].yAxisData"
                        :key="'a' + index" style="margin-bottom: 10px">
                        <el-input placeholder="请输入" size="small"
                            v-model="configData.chartOption.staticDataValue[0].yAxisData[index]"
                            style="width: calc(100% - 38px)" />
                        <el-button style="margin-left: 10px" size="mini" @click="delItem(index)" type="danger"
            ...
            <el-tab-pane label="x轴数据" name="xAxisData">
                ...
                <div style="max-height:500px;overflow-y:scroll;margin-top:10px">
                    <div v-for="(tmpitem, index) in configData.chartOption.staticDataValue[0].xAxisData"
                        :key="'a' + index" style="margin-bottom: 10px">
                        <el-input placeholder="请输入" size="small"
            ...
            <el-tab-pane label="数据" name="data">
                <!-- <el-scrollbar class="right-scrollbar"> -->
                <el-table border :data="configData.chartOption.staticDataValue[0].data" max-height="800"
                    style="margin-top: 10px;">
                    <el-table-column label="y轴序号" align="center" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row[0]" placeholder="请输入"
                                @input="inputYdata(scope.$index, 0, $event)" />
                        </template>
                    </el-table-column>
                    <el-table-column label="x轴序号" align="center" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row[1]" placeholder="请输入"
                                @input="inputYdata(scope.$index, 1, $event)" />
                        </template>
                    </el-table-column>
                    <el-table-column label="值" align="center" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row[2]" placeholder="请输入"
                                @input="inputYdata(scope.$index, 2, $event)" />
                        </template>
                    </el-table-column>

        ...
        <el-collapse-item title="动画配置" name="2">
          ...
          <!-- <el-form-item v-if="configData.chartOption.animationList!==undefined" label="动画列表"> -->
            <div class="animate_con">
              <div class="title"><span class="title_text">动画列表</span><el-button type="text" @click="addMove()">+ 添加动画</el-button></div>
              <div class="animate_li_con" v-for="(item,inx) in configData.chartOption.animationList" :key="'ani'+inx">
                <div class="name_con">
                  ...
                  <el-input v-model="configData.chartOption.animationList[inx].name" placeholder="请输入动画名称" />
                   <el-button style="margin-left: 10px" size="mini" @click="delMove(inx)" type="danger" icon="el-icon-delete" circle></el-button>
                </div>
                <div class="info_con">
                  <div><span>动画序列图：<span style="color:red;font-size:12px;margin-right:10px;">请上传10M以内的图片</span></span><el-button type="text" @click="addMoveImg(inx)">+ 添加序列图</el-button></div>
                  <div class="image_flex_con">
                    <div v-for="(ite,index) in configData.chartOption.animationList[inx].imglist" :key="'iotimg'+index" class="image_flex">
                      <div class="image_con"><image-upload :isShowTip="false" v-model="configData.chartOption.animationList[inx].imglist[index]" :limit="1"></image-upload></div>
                      <el-button style="margin-left: 10px" size="mini" @click="delMoveImg(index,inx)" type="danger" icon="el-icon-delete" circle></el-button>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <!-- <el-form-item label="动画条件" prop="conditionJson" style="width: 100%"> -->
            <div class="title"><span class="title_text" style="margin-right:20px;">条件</span><el-button type="text" @click="addConditionList()">+ 添加条件</el-button></div>
            <el-row :gutter="3" :key="'condition'+inx" v-for="(ite,inx) in configData.chartOption.conditionList" style="margin-top:10px;display:flex;align-items:center;">
              <el-col :span="1" style="font-size:12px">{{inx+1}}:</el-col>
              <el-col :span="7">
                <el-select v-model="configData.chartOption.conditionList[inx].field" placeholder="字段" filterable clearable>
                    <el-option v-for="item in fieldOptions" :key="item.filed" :label="item.name" :value="item.filed"></el-option>
                </el-select>
              </el-col>
              <el-col :span="8">
              ...
              <el-col :span="5">
                <el-input type="text" v-model="configData.chartOption.conditionList[inx].value" placeholder="值"></el-input>
              </el-col>
              <el-col :span="3">
                <el-button  size="mini" @click="delConditionsItem(inx)" type="danger" icon="el-icon-delete" circle></el-button>
              </el-col>
            </el-row>
            <div class="title"><span class="title_text" style="margin-right:20px;">条件组</span><el-button type="text" @click="addConditionGroupList()">+ 添加条件组</el-button></div>
            <template v-for="(item2,ix) in configData.chartOption.conditionGroup">
              <div class="num" :key="'group_num'+ix">
                <span style="margin-right:10px">条件组{{ix+1}}:</span><el-button type="text" @click="addConditionGroupCondition(ix)">+ 加条件</el-button>
                <el-button type="text" @click="delConditionGroupCondition(ix)" v-if="configData.chartOption.conditionGroup[ix].condition.length>0">- 减条件</el-button>
                <el-button  size="mini" @click="delConditionGroupList(ix)" type="danger" icon="el-icon-delete" circle></el-button>
              </div>
              <div style="display:flex;align-items:center;" :key="'groups'+ix">
                <el-row :gutter="3">
                    <el-col :span="18">
                      <el-row :gutter="3" v-if="configData.chartOption.conditionGroup[ix]&&configData.chartOption.conditionGroup[ix].condition.length>0">
                        <template v-for="(row,i) in configData.chartOption.conditionGroup[ix].condition">
                          <el-col :span="9" v-if="i==0" :key="'group_condition'+i">
                            <el-select v-model="configData.chartOption.conditionGroup[ix].condition[i]" placeholder="条件">
                              <el-option v-for="(item,dx) in configData.chartOption.conditionList" :key="'grCon'+dx" :label="'第'+Number(dx+1)+'个'" :value="dx"></el-option>
                            </el-select>
                          </el-col>
                    ...
                    <el-col :span="6" v-if="configData.chartOption.conditionGroup&&configData.chartOption.conditionGroup[ix].condition.length>0">
                      <el-select v-model="configData.chartOption.conditionGroup[ix].animation" placeholder="动画">
                        <el-option v-for="(item,dx) in configData.chartOption.animationList" :key="'grAn'+dx" :label="item.name" :value="dx"></el-option>
                      </el-select>
                    </el-col>
                </el-row>
              </div>

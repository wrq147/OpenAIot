...
<script>
import { matchesSelectorToParentElements, getComputedSize, addEvent, removeEvent } from '../../util/dom'
import { computeWidth, computeHeight, restrictToBounds, snapToGrid } from '../../util/fns'
...
export default {
  ...
  methods: {
    ...
    // 获取父元素大小
    getParentSize () {
      if (this.parent === true && this.$el.parentNode !== null) {
    ...
    calcDragLimits () {
      return {
        ...
        maxLeft: Math.floor((this.parentWidth) / this.grid[0]) * this.grid[0] + this.left, // Math.floor((this.parentWidth - this.width - this.right) / this.grid[0]) * this.grid[0] + this.left
        minRight: this.right % this.grid[0],
        maxRight: Math.floor((this.parentWidth) / this.grid[0]) * this.grid[0] + this.right, //  Math.floor((this.parentWidth - this.width - this.right) / this.grid[0]) * this.grid[0] + this.right
        minTop: this.top % this.grid[1],
        maxTop: Math.floor((this.parentHeight) / this.grid[1]) * this.grid[1] + this.top, // Math.floor((this.parentHeight - this.height - this.top) / this.grid[1]) * this.grid[1] + this.top
        minBottom: this.bottom % this.grid[1],
        maxBottom: Math.floor((this.parentHeight) / this.grid[1]) * this.grid[1] + this.bottom // Math.floor((this.parentHeight - this.height - this.bottom) / this.grid[1]) * this.grid[1] + this.bottom
      }
    },
    // 取消
    deselect (e) {
    ...
    //ctrl+点击
    onClickCtrl() {
      VueEvent.$emit("click_ctrl", this.customId);
    },
  },
  computed: {
    handleStyle () {

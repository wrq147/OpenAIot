  ...
  <div class="invite_register jiayuinvite_register">
    <el-form ref="inviteRegisterForm" :model="inviteRegisterForm" :rules="inviteRegisterRules" label-position="top" class="invite_register-form jiayu_form">
      ...
      <div class="form_con jiayuform_con" :style="{ width: showOrgInfo&&loginType=='default'?'calc(50% + 50px)':'100%' }">
        <div v-if="loginType=='default'" class="left_con" :style="{ width: showOrgInfo?'100%':'calc((100% - 50px) / 2)','margin-right':showOrgInfo?'0':'50px' }">

          <div class="invite_register_title" v-if="loginType=='default'">
            <h2 class="title">{{showOrgInfo?'员工邀请':'注册账号'}}</h2>
          </div>
          <div class="avatar_con" style="height:100%">
            <img v-if="loginType=='default'" :src="inviteRegisterForm.avatar" style="width: 100px; height: 100px; margin-bottom: 10px" @click.stop="changeDiffentAvatar"/>
            <div class="invite_name">{{inviteRegisterForm.realName}}</div>

  ...
  <el-dialog title="提示" :visible.sync="dialogVisible" width="800px" :before-close="handleClose">
    <el-form ref="form" :model="form" label-width="100px" :rules="rules">
      ...
      <el-form-item label="隐藏条件">
        ...
        <el-row :gutter="10" :key="'condition'+inx" v-for="(ite,inx) in form.conditions" style="margin-top:10px;margin-left:-60px">
          <el-col :span="2">
          ...
          <el-col :span="4">
            <el-select v-model="form.conditions[inx].field" placeholder="请选择字段" @change="fieldChange($event,inx)">
              <el-option v-for="item in activeTypeFiledList" :key="item.mapid" :label="item.name" :value="item.mapid"></el-option>
            </el-select>
          </el-col>
          <el-col :span="3">
            <el-select class="time_mini" placeholder="请选择比较符" v-model="form.conditions[inx].compare" v-if="form.conditions[inx]&&form.conditions[inx].valtype=='文本'">
            ...
            <el-select class="time_mini" @change="numberCompareChange($event,inx)" v-model="form.conditions[inx].compare" placeholder="请选择比较符" v-if="form.conditions[inx]&&form.conditions[inx].valtype=='数字'">
              <el-option v-for="item in compareList" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
            <el-select class="time_mini" @change="numberCompareChange($event,inx)" v-model="form.conditions[inx].compare" placeholder="请选择比较符" v-if="form.conditions[inx]&&form.conditions[inx].valtype=='时间'">
              <el-option v-for="item in timeCompareList" :key="item.value" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-col>
          <!-- <el-col :span="3">
          ...
          <el-col :span="2" v-if="form.conditions[inx]&&form.conditions[inx].valtype=='时间'" class="time_mini">
            <el-select @change="conditionValtype($event,inx)" v-model="form.conditions[inx].val.timeType" placeholder="请选择">
          ...
          <div v-if="form.conditions[inx].compare&&form.conditions[inx].compare!='ISNULL'&&form.conditions[inx].compare&&form.conditions[inx].compare!='NOTNULL'">
            <el-col :span="9" v-if="form.conditions[inx]&&form.conditions[inx].valtype=='数字'">
              <div class="number_val_con" v-if="form.conditions[inx].compare&&form.conditions[inx].compare=='INRANGE'||form.conditions[inx].compare&&form.conditions[inx].compare=='NOTINRANGE'">
                <el-input style="width:calc(50% - 10px)" type="number" v-model="form.conditions[inx].val.max" placeholder="最小值"></el-input>
                  <span>~</span>
                <el-input style="width:calc(50% - 10px)" type="number" v-model="form.conditions[inx].val.min" placeholder="最大值"></el-input>
              </div>
              <div v-else>
                <el-input type="number" v-model="form.conditions[inx].val" placeholder="请输入值"></el-input>
              </div>
            </el-col>
            <el-col :span="5" v-if="form.conditions[inx]&&form.conditions[inx].valtype=='时间'&&form.conditions[inx].compare&&form.conditions[inx].compare!='SELECTRANGE'&&form.conditions[inx].compare!='DYNAMICS'">
              <!-- <el-input type="text" v-model="form.conditions[inx].val" placeholder="请输入值"></el-input> -->
              <el-date-picker style="width:100%" v-model="form.conditions[inx].val.timeValue" type="date" placeholder="请选择值" v-if="form.conditions[inx].val.timeType&&form.conditions[inx].val.timeType=='日'"></el-date-picker>
              <el-date-picker style="width:100%" v-model="form.conditions[inx].val.timeValue" type="datetime" placeholder="请选择值" v-else :value-format="returnFormatDate(inx)" :format="returnFormatDate(inx)"></el-date-picker>
            </el-col>
            <el-col :span="11" v-if="form.conditions[inx]&&form.conditions[inx].valtype=='时间'&&form.conditions[inx].compare&&form.conditions[inx].compare=='SELECTRANGE'">
              <!-- <el-input type="text" v-model="form.conditions[inx].val" placeholder="请输入值"></el-input> -->
              <el-date-picker style="width:100%" v-model="form.conditions[inx].val.timeValue" type="daterange" placeholder="请选择值" v-if="form.conditions[inx].val.timeType&&form.conditions[inx].val.timeType=='日'"></el-date-picker>
              <el-date-picker style="width:100%" v-model="form.conditions[inx].val.timeValue" type="datetimerange" placeholder="请选择值" v-else :value-format="returnFormatDate(inx)" :format="returnFormatDate(inx)"></el-date-picker>
            </el-col>
            <el-col :span="11" v-if="form.conditions[inx]&&form.conditions[inx].valtype=='时间'&&form.conditions[inx].compare&&form.conditions[inx].compare=='DYNAMICS'">
              <!-- <el-input type="text" v-model="form.conditions[inx].val" placeholder="请输入值"></el-input> -->
               <el-date-picker :key="timeComptKey" style="width:100%" v-model="form.conditions[inx].val.timeValue" type="datetimerange" align="right" unlink-panels range-separator="至"
                start-placeholder="开始时间" end-placeholder="结束时间" :picker-options="pickerOptions" :value-format="returnFormatDate(inx)" :format="returnFormatDate(inx)">
              </el-date-picker>
            </el-col>
      ...
      <div class="CheckBoxField" v-if="form.type=='复选框'||form.type=='单选框'">
        ...
        <el-form-item label="可选项">
          <div class="options_con">
            ...
            <div class="option_input" v-if="showInput"><el-input v-model="activeSelectValue" ref="enumKeyAuto" placeholder="请输入选项" @blur="inputBlur" @change="inputBlur"/></div>
          </div>
        </el-form-item>
        ...
        <el-form-item label="填充规则">
            <el-card class="box-card">
                <div slot="header" class="clearfix">
                    <el-button :disabled="!form.object_type" style="float: left; padding: 3px 0" type="text" @click="addFillRules" class="el-icon-circle-plus-outline">填充规则</el-button>
                </div>
                <table>
                  <tr v-for="(it,ix) in form.items" :key="'rul'+ix">
                    ...
                    <td>
                      <el-select v-model="form.items[ix].source_obj" :placeholder="'请选择'+form.object_type+'的字段'">
                        <template v-if="objectFiledSelectListObj[form.object_type]&&objectFiledSelectListObj[form.object_type].length>0">
                          <el-option :label="its.name" :value="its.mapid" v-for="its in objectFiledSelectListObj[form.object_type]" :key="'sourcesel'+its.mapid"></el-option>
                        </template>
                      </el-select>
                    </td>
                    <td>
                    ...
                    <td>
                      <el-select v-model="form.items[ix].field" :placeholder="'请选择'+customFiledType+'的字段'">
                        <template v-if="objectFiledSelectListObj[customFiledType]&&objectFiledSelectListObj[customFiledType].length>0">
                          <el-option :label="its.name" :value="its.mapid" v-for="its in objectFiledSelectListObj[customFiledType]" :key="'sourcesel'+its.mapid"></el-option>
                        </template>
                      </el-select>
                    </td>
                  </tr>
      ...
      <div class="timeFiled" v-if="form.type=='时间'">
        ...
        <el-form-item label="默认值" v-if="form.format">
          <el-date-picker v-model="form.defvalTime" type="datetime" placeholder="选择" :value-format="form.format" :format="form.format"></el-date-picker>
        </el-form-item>
      </div>
    </el-form>
   ...
   async openDialog(index){
    ...
    setMapid(){
        ...
        try {
            ...
            if(filedTableList&&filedTableList.length>0){
              if(this.form.type=='数字'||this.form.type=='时间'){
                ...
                if(filterArr){
                  mapidArr=(filterArr.map(row=>{
                    if(row.mapid){
                      // console.log(row.mapid.slice(6),'row.mapid.slice(5)');
                      return Number(row.mapid.slice(6))
                    }
                  })).filter(rw=>rw!=undefined)
                }
          ...
          // console.log(this.form.mapid,'this.form.mapid');
        } catch (error) {

    ...
    <div class="drawer_content_con" id="drawer_content_con">
      <el-tabs tab-position="left" v-model="activeName">
        ...
        <el-form ref="form" :model="form" label-width="80px" class="baseinfo_con" :rules="rules">
          ...
          <el-form-item label="二级分组" v-show="activeName=='baseinfo'" prop="levelCode">
            <el-select v-model="form.levelCode" placeholder="请选择字段" clearable>
              <template v-for="item in selectTypeFiled">
                <el-option :key="item.mapid" :label="item.name" :value="item.mapid"></el-option>
              </template>
            </el-select>
          </el-form-item>
        ...
        <div class="filed_table" v-show="activeName=='filedsetting'">
          ...
          <div class="zhuyi"><i class="el-icon-info" style="margin-right: 5px"></i>设置此对应表分组下字段的顺序、显隐和固定</div>
          <div class="btn_con">
...
export default {
  ...
  data() {
    return {
      ...
      productFiledList:[],//对应表字段
      rules: {
        name: [{ required: true, trigger: "blur", message: "对应表分组名称不能为空" },],
        photoUrl: [{ required: true, trigger: "blur", message: "对应表分组图片不能为空" },],
      },
      associationObject:{},//所有关联对象对应的下拉的参数列表
      selectTypeFiled:[],//单选框类型的字段
      tableName:'',//表名称
    };
  },
  computed:{
  ...
  methods: {
    ...
    async getorgFormFields(beforeVal){
      ...
      try {
        this.fieldTable=[]
        let res=await orgFormFields({field:this.tableName,ext:true})
        // console.log("对应表所有字段",res);
        let data=res.data
        this.productFiledList=JSON.parse(JSON.stringify(res.data))
        this.selectTypeFiled=this.productFiledList.filter(ro=>ro.type=='单选框')
      ...
      this.$refs["form"].validate((valid,validateResult) => {
        if(valid){
          ...
          let submitform={
            ...
            levelCode:this.form.levelCode?this.form.levelCode:'',
          ...
          if(this.form.id){
            ...
            editGroupViewSave(submitform).then(res=>{
            ...
            addGroupViewSave(submitform).then(res=>{
    ...
    async setDrawOpen(id,tableName){
      this.tableName=tableName
      if(id){
        this.title='修改对应表分组'
        let res=await groupViewInfo({id:id})
        // console.log(res,'resres');
        this.form={
          id:id,
          sort:res.data.Sort,
    ...
    async setSaveFilter(filterList,tableName){
      //另存新视图的操作
      this.tableName=tableName
      this.filterList=JSON.parse(JSON.stringify(filterList))
      this.fieldTable=[]

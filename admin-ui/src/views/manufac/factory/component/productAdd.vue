          ...
          <el-col :span="12" :key="'custom_filed' + ix" v-if="!inSetFormItemHide(item, form.ExtVals)">
            <el-form-item :label="item.name" :prop="item.mapid">
              <el-select @change="customValChange($event, item)" :disabled="item.is_readonly" :filterable="item.is_add"
                ...
                v-model="form.ExtVals[item.mapid]" :placeholder="item.prompt_text ? item.prompt_text : '请选择'"
                style="width: 100%"
              ...
              <el-radio-group @change="customValChange($event, item)" :disabled="item.is_readonly"
                v-model="form.ExtVals[item.mapid]" v-if="item.type == '单选框' && item.show_way == '平铺'">
                <template v-for="it in item.optionals">
                  <el-radio :label="it" :key="it + ix">{{ it }}</el-radio>
                </template>
              </el-radio-group>
              <el-checkbox-group @change="customValChange($event, item)" :disabled="item.is_readonly"
                v-model="form.ExtVals[item.mapid]" v-if="item.type == '复选框' && item.show_way == '平铺'">
                <template v-for="it in item.optionals">
              ...
              <el-date-picker @blur="customValChange($event, item)" @change="customValChange($event, item)"
                :disabled="item.is_readonly" v-if="item.type == '时间'" v-model="form.ExtVals[item.mapid]" type="datetime"
    ...
    async openDialog(id) {//添加弹窗打开
      try {
        ...
        if (id) {
          ...
          setCustomDefaultValue(this.filedTableList, this.form, this.rules, productinfo);

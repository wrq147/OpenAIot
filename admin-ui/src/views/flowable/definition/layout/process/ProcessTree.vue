<script>
//导入所有节点组件
import AllNodes from "./ProcessTreeExport"
...
export default {
  ...
  computed:{
    ...
    dom(){
      return this.$store.state.flowable.design.process;
    }
  },
  render(h, ctx) {
    this.nodeMap.clear()
  ...
  methods: {
    ...
    decodeAppendDom(h, node, dom, props = {}){
      if(node.type=="USER"||node.type=="APPROVAL"){//处理optionInit初始化问题
        if(node.props.optionInit==undefined){
          this.$set(node.props,'optionInit',[])
        }
      }
      props.config = node
    ...
    isPrimaryNode(node){
      return node &&
          (node.type === 'ROOT' || node.type === 'APPROVAL' || node.type === 'USER'
          || node.type === 'CC' || node.type === 'DELAY'
          || node.type === 'TRIGGER'|| node.type === 'DATAXE'|| node.type === 'FUNC');
    },
    isBranchNode(node){
    ...
    insertNode(type, parentNode){
      ...
      switch (type){
        ...
        case 'USER': this.insertUserNode(parentNode); break;
        ...
        case 'DATAXE':this.insertDataNode(parentNode); break;
        case 'FUNC':this.insertFuncNode(parentNode); break;
        default: break;
      }
      //拼接后续节点
    ...
    insertUserNode(parentNode){
      this.$set(parentNode.children, "name", "办理人")
      this.$set(parentNode.children, "props", deepCopy(DefaultProps.USER_PROPS))
    },
    ...
    insertDataNode(parentNode){
      this.$set(parentNode.children, "name", "数据执行")
      this.$set(parentNode.children, "props", deepCopy(DefaultProps.DATA_AC_PROPS))
    },
    insertFuncNode(parentNode){
      this.$set(parentNode.children, "name", "设备执行")

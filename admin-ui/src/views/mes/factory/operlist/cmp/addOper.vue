<template>
    <el-dialog v-if="dialogFlag" :title="title" :visible.sync="dialogFlag" :close-on-click-modal="false" append-to-body
            ...
            <el-col :span="12">
                <el-form-item label="工序使用的设备">
                    ...
                    <div v-if="equipment.length > 0">
                        <el-tag v-for="(item, index) in equipment" :key="index" style="margin-right: 10px" closable @close="removeEmployee('equipment', index)">
                            {{ item.Name }}
                        </el-tag>
                    </div>
                </el-form-item>
        ...
        <el-row v-show="dialogName === 'baseinfo'">
            ...
            <el-col :span="24">
                <el-form-item label="报工表单权限">
                    <el-table :data="reportFieldsData" max-height="200" style="width: 100%;" row-key="Id" class="data_table" border>
            ...
            <el-col :span="24">
                <el-form-item label="报工表单初始化配置">
                    ...
                    <el-table :data="fieldsInit" max-height="200" style="width: 100%;" row-key="Id" class="data_table" border>
                        <el-table-column label="工序表单字段" align="center">
                            <template slot-scope="scope">
                                <el-select v-model="scope.row.sid" filterable clearable placeholder="请选择" @change="handleSourceChange(scope.$index, scope.row)">
                                    <el-option
                                        v-for="(item, index) in filteredSourceOptions(scope.row.tid)"
                                        :key="index"
                                        :label="item.name"
                                        :value="item.mapid"
                                    />
                        ...
                        <el-table-column label="报工表单字段" align="center">
                            <template slot-scope="scope">
                                <el-select v-model="scope.row.tid" filterable clearable placeholder="请选择" @change="handleTargetChange(scope.$index, scope.row)">
                                    <el-option
                                        v-for="(item, index) in filteredTargetOptions(scope.row.sid)"
                                        :key="index"
                                        :label="item.name"
                                        :value="item.mapid"
                                    />
            ...
            <el-col :span="12" :key="'custom_filed' + ix" v-if="!setFormItemHide(item)">
              <el-form-item :label="item.name" :prop="item.mapid">
                <el-select @change="customValChange" :disabled="item.is_readonly" :allow-create="item.is_add" :multiple="item.type === '复选框'" :clearable="!item.is_required"
                  v-model="ruleForm[item.mapid]" :placeholder="item.prompt_text ? item.prompt_text : '请选择'" style="width: 100%"
                  v-if=" (item.type === '单选框' && item.show_way === '下拉') || (item.type == '复选框' && item.show_way === '下拉') ">
                    <el-option v-for="it in item.optionals" :label="it" :value="it" :key="it + ix"></el-option>
                </el-select>
                <el-radio-group @change="customValChange" :disabled="item.is_readonly" v-model="ruleForm[item.mapid]" v-if="item.type === '单选框' && item.show_way === '平铺'">
                    <el-radio v-for="it in item.optionals" :label="it" :key="it + ix">{{ it }}</el-radio>
                </el-radio-group>
                <el-checkbox-group @change="customValChange" :disabled="item.is_readonly" v-model="ruleForm[item.mapid]" v-if="item.type === '复选框' && item.show_way === '平铺'">
                    <el-checkbox v-for="it in item.optionals" :label="it" :key="it + ix">{{ it }}</el-checkbox>
                </el-checkbox-group>
                <el-date-picker @change="customValChange" :disabled="item.is_readonly" v-if="item.type === '时间'" v-model="ruleForm[item.mapid]"
                ...
                <el-input @input="customValChange" :disabled="item.is_readonly" v-if="item.type === '文本'" :placeholder="item.prompt_text ? item.prompt_text : '请输入'"
                  :type="item.is_multiple ? 'textarea' : 'text'" v-model="ruleForm[item.mapid]"></el-input>
                <el-input @input="customValChange" :disabled="item.is_readonly" v-if="item.type === '数字'" :placeholder="item.prompt_text ? item.prompt_text : '请输入'"
                  type="number" v-model="ruleForm[item.mapid]" :precision="item.decimals"></el-input>
      ...
      <add-Equipment ref="addEquipment" :dialog-visible="equipOpen" :equipment="equipment" @cancelForm="cancelForm" @getSelectEquip="getSelectEquip" />
    </el-dialog>
</template>
<script>

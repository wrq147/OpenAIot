<template>
  <el-dialog v-if="dialogFlag" :title="title" :visible.sync="dialogFlag" :close-on-click-modal="false" append-to-body
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="计划名称" prop="planName">
            <el-input v-model="ruleForm.planName" placeholder="请输入计划名称" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="超期时间" prop="overTime">
            <el-date-picker v-model="ruleForm.overTime" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm"
              type="datetime" placeholder="选择超期时间" />
          </el-form-item>
        </el-col>
    ...
    <el-table :data="ruleForm.items" border tooltip-effect="dark" style="width: 100%">
      ...
      <el-table-column label="产品编号" prop="SkuNumber" align="center" width="150"></el-table-column>
      ...
      <el-table-column align="center" width="210">
        ...
        <template slot-scope="scope">
          <el-input-number v-model="scope.row.Quantity" :min="1" :max="9999999"></el-input-number>
        </template>
      </el-table-column>
      <el-table-column align="center" width="200">
        ...
        <template slot-scope="scope">
          <el-date-picker v-model="scope.row.PlannedStartOn" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm"
            type="datetime" placeholder="选择计划开始时间" />
        </template>
      </el-table-column>
      <el-table-column align="center" width="200">
        <template #header>
        ...
        <template slot-scope="scope">
          <el-date-picker v-model="scope.row.PlannedEndOn" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm"
    ...
    getDataInfo(id) {
      operInfo({ id }).then(res => {
        this.ruleForm.items = res.data.Items.map(row => {
          let dataObj = {
            SkuNumber: row.ProdInfo.SkuNumber,

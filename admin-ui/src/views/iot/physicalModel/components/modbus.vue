      ...
      <el-row>
        <el-col :span="10" style="padding-right: 30px; box-sizing: border-box">
          ...
          <el-form-item label="Mode" prop="Mode">
            <el-select v-model="modbussForm.Mode" placeholder="请选择Mode" style="width: 100%">
              <el-option v-for="ite in ModeList" :key="ite.value" :label="ite.label" :value="ite.value"></el-option>
            </el-select>
          </el-form-item>
            ...
            <div v-if="modbussForm.Matches && modbussForm.Matches.length > 0" class="param_list_con">
              <div class="param_list" v-for="(item, inx) in modbussForm.Matches" :key="inx">
                ...
                <div class="list_right">
                  ...
                  <i class="el-icon-edit" @click="editOthersParams(item, inx)"></i>
                  <i class="el-icon-delete" @click="deleteOthersParams(item, inx)"></i>
                </div>
              </div>
            </div>
            <span style="color: #0055ff; cursor: pointer" @click="openMatchesDrawer">+增加规则</span>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      activeAddParamsIndex: -1, //当前修改的规则是哪一行
    };
  },

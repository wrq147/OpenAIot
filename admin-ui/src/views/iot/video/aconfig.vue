<template>
  <div class="ai-config-container">
      ...
      <div class="config-layout">
        ...
        <div class="config-column">
          ...
          <div v-if="filteredOptionalProjects.length === 0" class="empty-state">
            <el-empty
              description="暂无可选项目<br/>所有项目已添加至配置列表"
              :image-size="120"
            >
              <el-button
                type="text"
        ...
        <div class="config-column">
          ...
          <div v-if="configuredProjects.length === 0" class="empty-state">
            <el-empty
              description="暂无已配置项目<br/>从左侧选择项目添加"
              :image-size="120"
            />
          </div>
    ...
    <!-- 参数配置弹窗 -->
    <el-dialog
      :title="`${currentProject?.Name} - 参数配置`"
...
export default {
  ...
  created() {
    ...
    this.filteredOptionalProjects = [...this.optionalProjects]
  },
  methods: {
    /**
     * 初始化可选项目列表
    ...
    handleOptionalSearch() {
      ...
      this.filteredOptionalProjects = this.optionalProjects.filter(project => {
        return project.Name.toLowerCase().includes(this.optionalSearchText.toLowerCase())
      })
    },
    ...
    async addSelectedProjects() {
      ...
      try {
        ...
        // 添加项目并去重
        this.configuredProjects.push(...this.selectedProjects)
     ...
     */
    openParamConfig(row) {
      this.currentProject = { ...row } // 深拷贝防止实时联动
        ...
        if (row.paramValues && row.paramValues[param.code] !== undefined) {
          ...
          switch (param.type) {
            ...
            case 'boolean':
              ...
              break
            case 'enum':
              this.paramFormData[param.code] = param.options?.[0]?.value || ''
              break
            case 'string':
    ...
    async saveParamConfig() {
      try {
        ...
        if (targetProject) {
          targetProject.paramValues = { ...this.paramFormData }
        }

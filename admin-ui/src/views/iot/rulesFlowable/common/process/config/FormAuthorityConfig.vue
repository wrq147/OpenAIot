...
export default {
  ...
  computed: {
    ...
    formData() {
      return this.$store.state.rulesFlowable.rulesDesign.formItems
    },
    formPerms() {
      return this.$store.state.rulesFlowable.rulesSelectedNode.props.formPerms
    }
  },
  methods: {
    allSelect(type) {
        ...
        if (form.name === 'SpanLayout') {
          ...
          if (old){
            ...
            this.formPerms.push({
              ...
              perm: this.$store.state.rulesFlowable.rulesSelectedNode.type === 'ROOT' ? 'E' : 'R'
            })
          }
        }
      })

<template>
  <div>
    <el-form inline label-width="100px">
      <el-form-item label="调整优先级" prop="level">
        <el-popover placement="right" title="拖拽条件调整优先级顺序" width="250" trigger="click">
          ...
          <el-button icon="el-icon-sort" slot="reference">第{{ nowNodeLeave + 1 }}级</el-button>
        </el-popover>
      </el-form-item>
    ...
    <div style="width:100%">
      ...
      >添加条件组</el-button>
      <span class="item-desc">只有物模型属性与一些系统信息才能作为条件</span>
    </div>
    <group-item v-if="showchild" />
  </div>
</template>
...
export default {
  ...
  components: { Ellipsis, draggable, GroupItem },
  props: {
    options: {
      type: Object,
      default() {
  ...
  computed: {
    ...
    //条件节点
    prioritySortList() {
      let node = this.$store.state.rulesFlowable.rulesNodeMap.get(
  ...
  methods: {
    addConditionGroup() {
      if (this.config.groups.length >= 10) {
        this.$message.warning("条件组不能超过10个");
        return;
      }

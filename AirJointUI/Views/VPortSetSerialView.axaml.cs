...
public partial class VPortSetSerialView : UserControl
{
    public VPortSetSerialView()
    {
        InitializeComponent();
    ...
    private async void HandleGotFocus(object sender, GotFocusEventArgs e)
    {
        if (Interlocked.CompareExchange(ref _isTab, 1, 0) == 0)
        {
            ...
            hostControl.Focusable = false;
            var topLevel = (Window)TopLevel.GetTopLevel(this);
            var result = await new VPortNumberKeyboard(hostControl.Text).ShowDialog<string>(topLevel);
            if (result != null)
            {
                int tmprs;

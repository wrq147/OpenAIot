...
{
    public class MySqlCompatible : ICompatible
    {
        private MySqlHelp _db;
        private SqlBuilder _sql;
        public MySqlCompatible(MySqlHelp db, SqlBuilder sql)
        {
            _db = db;
            _sql = sql;
        }
        ...
        {
            _sql.ReplaceLeftOne("select", "select SQL_CALC_FOUND_ROWS");
            SqlPage(page, size, orderby);
            _sql.Append(";select FOUND_ROWS()");
        }
        public void SqlPage(int page, int size, string orderby)
        {
            ...
            _sql.Append(string.Format(" limit {0},{1}", start_row, size));
        }
        ...
        public long DoExecReturnIdentity(ExcuteParam p, string idname)
        {
            ...
            return ((MySqlCommand)p.Command).LastInsertedId;
        }

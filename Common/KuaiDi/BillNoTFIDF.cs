...
namespace Common.KuaiDi
{
    public class TFIDFMeasure
    {
        ...
        private string[] Tokeniser(string input)
        {
            List<string> stlist = new List<string>();
            string[] words = input.Split(' ', StringSplitOptions.RemoveEmptyEntries);
            foreach(string w in words)
            {
                stlist.Add(w.Substring(0, 2));
                stlist.Add(w.Substring(0, 3));
                stlist.Add(w.Substring(0, 4));
                stlist.Add(w.Substring(0, 5));
                if (w.Length > 8)
                {
                    stlist.Add(w.Substring(0, 6));
                    stlist.Add(w.Substring(0, 7));
                    stlist.Add(w.Substring(0, 8));
                }
            }
        ...
        private IDictionary GetWordFrequency(string input)
        {
            ...
            String[] words = Tokeniser(convertedInput);
            Array.Sort(words);

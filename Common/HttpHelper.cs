...
namespace Common
{
    ...
    public class HttpHelper
    {
        ...
        {
            url += "?" + string.Join("&", getparams.Select(x => $"{x.Key}={Uri.EscapeDataString(x.Value)}"));
            var response = await _client.GetAsync(url);
            var responseBytes = await response.Content.ReadAsByteArrayAsync();
            return encode.GetString(responseBytes);
        }
        ...
        public async Task<string> PostWithHeaderAsync(string url, IEnumerable<KeyValuePair<string, string>> postparams, IEnumerable<KeyValuePair<string, string>> headers, Encoding encode)
        {
            FormUrlEncodedContent formContent = new FormUrlEncodedContent(postparams);
            foreach (var hitem in headers)
            {
                formContent.Headers.TryAddWithoutValidation(hitem.Key, hitem.Value);
            }
            HttpResponseMessage response = await _client.PostAsync(url, formContent);
            var strBytes = await response.Content.ReadAsByteArrayAsync();
            return encode.GetString(strBytes);
        ...
        public async Task<string> PostJsonWithHeaderAsync(string url, string json, IEnumerable<KeyValuePair<string, string>> headers, Encoding encode)
        {
            var content = new StringContent(json, encode, "application/json");
            foreach (var hitem in headers)
            {
                content.Headers.TryAddWithoutValidation(hitem.Key, hitem.Value);
            }

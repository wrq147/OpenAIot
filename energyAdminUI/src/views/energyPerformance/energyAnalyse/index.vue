...
import { facilityEnergyPageDateList, productionPageDateList } from '@/api/energy/energyMeter';
import { facilityProductList } from '@/api/energy/product';
import factorType from '../../energyMeter/energyCellStatis/cmp/factorType.vue';
import searchData from './cmp/searchData.vue';
import { processEnergyData } from '@/views/energyMeter/energyCompositeSearch/cmp/searchFunction';
export default {
    name: 'energyAnalyse',
    components: {
      factorType,
    ...
    methods: {
        // 获取数据
        getInitList() {
            facilityEnergyPageDateList(this.queryParams).then(response => {
                productionPageDateList(this.queryParams).then(async res => {
                    this.tableData = await processEnergyData(response.data.List, res.data.List[0]);
                })
            })
        },
        getList(currentKey) {
            this.queryParams.facilityId = currentKey ? currentKey : this.queryParams.facilityId;
            facilityProductList(this.queryParams).then(res => {

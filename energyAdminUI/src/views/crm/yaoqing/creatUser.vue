<template>
  <div class="invite_register jiayuinvite_register">
    <el-form ref="inviteRegisterForm" :model="inviteRegisterForm" :rules="inviteRegisterRules" label-position="top" class="invite_register-form jiayu_form">
      ...
      <div class="form_con jiayuform_con">
        <div class="left_con" v-if="loginType=='default'">
          ...
          <div class="avatar_con">
            <img :src="inviteRegisterForm.avatar" style=" width: 100px; height: 106.5px; border-radius: 50%; margin-bottom: 10px;" @click.stop="changeDiffentAvatar"/>
            <div class="svg-wrap" style="width: 100px;height: 100px;border-radius: 50%;margin-bottom: 10px;position: fixed;left: 200%;top: 200%;">
              <avataaars :key="timer"></avataaars>
            </div>
            <el-upload class="avatar-uploader" ref="upload" action="/" :show-file-list="false" :before-upload="handleBeforeUpload" :http-request="handMove">
              <div>
                <span class="upload_click">点击上传头像</span>
              </div>
            </el-upload>
          </div>
        </div>
        <div class="right_con">
          ...
          <el-form-item prop="realName" v-if="loginType=='default'">
            <span slot="label">
              <i slot="prefix" class="zhongtaiiconfont zhongtai-icon-zhanghao" style="margin-right: 14px;color: #C1C1C1"></i>姓名</span>
            <el-input v-model="inviteRegisterForm.realName" type="text" auto-complete="off" placeholder="请输入姓名">
            </el-input>
          </el-form-item>
          <el-form-item prop="mobile" label-width="100%" v-if="isphoneReg" class="qiehuan_label">
            <div class="form_label_con" slot="label" v-if="loginType=='default'">
              <div class="label_left">
                <i slot="prefix" class="zhongtaiiconfont zhongtai-icon-shoujihaoma" style="margin-right: 14px;color: #C1C1C1"></i>手机号
              </div>
              <div v-if="canEmailReg&&!node" class="label_right" style="cursor: pointer" @click.stop="changeRegType(false)">
          ...
          <el-form-item prop="email" label-width="100%" v-if="!isphoneReg && canPhoneReg" class="qiehuan_label">
            <div class="form_label_con" slot="label" v-if="loginType=='default'">
              <div class="label_left">
                <i slot="prefix" class="zhongtaiiconfont zhongtai-icon-shoujihaoma" style="margin-right: 14px;color: #C1C1C1"></i>邮箱
              </div>
          ...
          <el-form-item prop="mobileCode" v-if="captchaOnOff && isphoneReg&&!node">
          ...
          <el-form-item prop="readAgreement">
            <el-checkbox-group v-model="inviteRegisterForm.readAgreement" style="height: 26px">
              <el-checkbox style="margin: 0px 0px 22px 0px">
                <span style="font-size: 12px">已阅读或同意<span style="color: rgba(24, 144, 255, 1)">服务协议</span>
                </span>
              </el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          ...
          <el-form-item style="width: 100%; padding-top: 0; margin-bottom: 0">
            <el-button :loading="loading" size="medium" type="primary" style="width: 100%" @click.native.prevent="handleinviteRegister('inviteRegisterForm')">
              <span v-if="!loading">{{node?'完成创建':'下一步'}}</span>
    ...
    <el-dialog title="填写图形验证码" class="imgCodeTips" :show-close="false" :visible.sync="imgCodeShow" width="400px" :modal-append-to-body="false" :close-on-click-modal="false" center>
      <div class="img_code_con">
        <el-input v-model="imgCode" type="text" auto-complete="off" placeholder="请输入图形验证码" style="width: 240px"></el-input>
...
import {agentInviteInfo,agentInviteReg} from "@/api/manufac/agentMansge";
...
export default {
  ...
  data() {
    return {
      ...
      canPhoneReg: false,
      activeLoading: false,
      activeEmailText: "", //邮箱号码
      activeDialogVisible: false, //是否打开邮箱激活弹窗
      activeDaoji: false, //是否开始倒计时
      daojiNum: 0, //邮箱验证有效时间倒计时
      isAlreadyReg: false, //是否已经注册过
      loginType:'default',
      node:'',//短信邀请的验证码
  ...
  methods: {
    ...
    loadYaoqingInfo(id){
      agentInviteInfo({id:id}).then(res=>{
        console.log("邀请信息",res);
        let data=res.data
        if(data){
          this.inviteInfo=data
          this.inviteRegisterForm.realName=data.ContactName
          
          if(this.node){
            this.changeRegType(true)
            this.inviteRegisterForm.mobile=data.Tel
          }else{
            if(this.isEmail(data.Tel)){
              this.changeRegType(false)
              this.inviteRegisterForm.email=data.Tel
            }else{
              this.changeRegType(true)
              this.inviteRegisterForm.mobile=data.Tel
            }
    ...
    changeRegType(val) {
      //切换注册方式
      this.isphoneReg = val;
      this.resetForm("inviteRegisterForm");
    },
    // 上传图片前校检格式和大小
    ...
    changeDiffentAvatar() {
      ...
      this.$nextTick(() => {
        ...
        canvas.height = (bb.width * 106.5) / 100;
        let context = canvas.getContext("2d");
        let _this = this;
        image.onload = function () {
          context.drawImage(image, 0, 0, bb.width, (bb.width * 106.5) / 100);
    ...
    handleinviteRegister(inviteRegisterForm) {
      ...
      this.$refs[inviteRegisterForm].validate((valid) => {
        ...
        if (valid) {
          ...
          if(this.node){
            agentInviteReg({
              realName:this.inviteRegisterForm.realName,
              mobile:this.inviteRegisterForm.mobile,
              avatar:this.inviteRegisterForm.avatar,
              password:this.inviteRegisterForm.password,
              deptId: 0,
              email: "",
              postName: "",
              mobileCode:this.node,
              ...
              if (res.code == 0) {
                ...
                this.$router.push("/?yaoqingId=" + this.yaoqingId + "&hailogin=" + true);
              }
            })
          }else{
            if (this.isphoneReg) {
              ...
              memberLogon(submitQuey).then((res) => {
                if (res.code == 0) {
                  ...
                  if (this.code && !this.yaoqingId) {
                    this.$router.push("/?code=" + this.code + "&hailogin=" + true);
    ...
    async confirm() {
      ...
      if (!infoRsp.EmailActive) {
        ...
        this.activeEmailText = this.loginForm.email;
        this.activeDialogVisible = true;
      } else {
        ...
        if (this.code && !this.yaoqingId) {
          ...
          this.$router.push(
            "/?yaoqingId=" + this.yaoqingId + "&hailogin=" + true
          );
        }
      }
    },

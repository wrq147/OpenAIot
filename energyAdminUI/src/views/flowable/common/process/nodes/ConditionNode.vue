          ...
          switch (subCondition.valueType) {
            case ValueType.date:
              if(subCondition.compare=="before"){
                subConditionStr=`${subCondition.title}在[${subCondition.value}]之前`;
              }
              else{
                subConditionStr=`${subCondition.title}在[${subCondition.value}]之后`;
              }
              break;
            case ValueType.dept:
            case ValueType.user:
            ...
            case ValueType.boolean:
              subConditionStr=`${subCondition.title}${subCondition.compare}${this.getDefault(subCondition.value[0], '')}`
              break;
          }

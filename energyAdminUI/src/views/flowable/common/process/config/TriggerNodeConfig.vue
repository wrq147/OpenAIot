    ...
    <el-form label-position="top" label-width="90px">
      ...
      <div v-if="config.type === 'WEBHOOK'">
        ...
        <el-form-item label="接口请求参数" prop="text">
          ...
          <div v-for="(param, index) in config.http.xparams" :key="param.name">
            ...
            <el-select v-if="param.isField" style="width: 180px" v-model="param.value"
              size="small" placeholder="请选择表单字段">
              <el-option v-for="form in forms" :key="form.id" :label="form.title" :value="form.id"></el-option>
            </el-select>
            <el-input v-else placeholder="请设置字段值" size="small"
              v-model="param.value" style="width: 180px"/>
        ...
        <el-form-item label="请求结果处理" prop="text">
          ...
          <span class="item-desc" v-if="config.http.handlerByScript">
            ...
            <div>
              支持函数<span style="color: dodgerblue">setFormByName(<span style="color: #939494">'表单字段名', '表单字段值'</span>)</span >可改表单数据
            </div>
          </span>
      ...
      <div v-else-if="config.type === 'EMAIL'">
        ...
        <el-form-item label="收件方" prop="text">
          <el-select size="small" style="width: 100%" v-model="config.email.to" filterable
            multiple allow-create default-first-option placeholder="请输入收件人">
            <el-option v-for="item in config.email.to"
              :key="item" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="邮件正文" prop="text">
          <el-input type="textarea" v-model="config.email.content"
            :rows="4" placeholder="邮件内容，支持变量提取表单数据 ${表单字段名} "></el-input>
        </el-form-item>
      </div>
      <div v-else-if="config.type === 'NEWFLOW'">
        ...
        <el-form-item label="目标流程自选人" prop="text" v-if="config.flow.templateId&&config.flow.NewAssignUser&&config.flow.NewAssignUser.length>0">
          <div class="item_row" v-for="(rowIt,inx) in config.flow.NewAssignUser" :key="'assign_user'+inx" :style="{ 'margin-top': inx>0?'5px':0 }">
            <el-input v-model="config.flow.NewAssignUser[inx].NodeName" placeholder="请输入内容" :disabled="true" style="width:180px"></el-input>
            <span>-</span>
            <el-select v-model="config.flow.NewAssignUser[inx].fieldid" placeholder="请选择">
              <el-option v-for="item in personSelectForm" :key="item.id" :label="item.title" :value="item.id">
              </el-option>
            </el-select>
          </div>
        </el-form-item>
        <el-form-item label="目标流程表单初始化" prop="text" v-if="config.flow.templateId&&config.flow.NewFlowItem&&config.flow.NewFlowItem.length>0">
          <div class="item_row" v-for="(rowIt,inx) in config.flow.NewFlowItem" :key="'form_user'+inx" :style="{ 'margin-top': inx>0?'5px':0 }">
            <el-input v-model="config.flow.NewFlowItem[inx].title" placeholder="请输入内容" :disabled="true" style="width:180px"></el-input>
            <span>-</span>
            <el-select v-model="config.flow.NewFlowItem[inx].fieldid" placeholder="请选择">
              <el-option v-for="item in getMappingForm(config.flow.NewFlowItem[inx].eltype)" :key="item.id" :label="item.title" :value="item.id">
              </el-option>
            </el-select>
          </div>
        </el-form-item>
        <el-form-item label="目标流程的流程编码赋值给当前表单" prop="text" v-if="config.flow.templateId">
          <el-select v-model="config.flow.flowas" placeholder="请选择">
...
export default {
  ...
  methods: {
    ...
    async loadTemplateInfo(id,isfirstload){
      ...
      if(isfirstload){}else{
        if(this.config.flow&&this.config.flow.NewAssignUser){
          this.config.flow.NewAssignUser=[]
        }
        if(this.config.flow&&this.config.flow.NewFlowItem){
          this.config.flow.NewFlowItem=[]
        }
        for(let i=0;i<this.allFormFields.length;i++){
          ...
          this.config.flow.NewFlowItem.push(rowPush)
        }
    ...
    loadFlowTree(row){
      ...
      if(row.props&&row.props.assignedType&&row.props.assignedType=='SELF_SELECT'){//子流程自选人
        ...
        this.config.flow.NewAssignUser.push(rowPush)
      }
    ...
    listDefinition() {
      definitionList({IsEmbed:false}).then(response => {
        ...
        response.data.map(row=>{
          this.processOptions=[...this.processOptions,...row.Items]
        })
      });
    },
    initScript(){

    ...
    <div class="factorLibrary-tree">
        <div class="factorLibrary-tree-input">
            <div class="factorLibrary-tree-info">
                ...
                <el-input @change="treeSearch" v-model="filterText" type="text" placeholder="请输入设施名称" clearable @clear="getInitList" />
            </div>
        </div>
        <el-tree 
            ref="treeSelect"
    ...
    methods: {
        ...
        getInitList() {
            facilityTree({ OrgId: this.$store.getters.orgId }).then(res => {
                this.facilityTreeList = res.data;
                this.$nextTick(() => {
                    if (res.data && res.data.length > 0) {
                        this.currentKey = res.data[0].Id;
                        this.$refs['treeSelect'].setCurrentKey(this.currentKey);
                        this.$emit('getList', this.currentKey,res.data[0].FacilityName);
                    }
                });
            })
        },
        handleNodeClick(data) {
            ...
            this.$emit('getList', this.currentKey,data.FacilityName);
        },
        // tree搜索
        treeSearch() {

      ...
      <div class="table_pie">
        ...
        <div class="pice_con">
            <div class="trend_title">
              <div class="line"></div>
              <div class="text">{{totalType=='mom'?'环比排名':'同比排名'}}</div>
...
export default {
        ...
        handler(val){
          if(val){
            ...
            if(newVal&&newVal.Times&&newVal.Times.length>0){
              ...
              this.currentBarData=this.processTrendData(Times)
            }else{
              this.currentBarData=[]
            }
  ...
  methods: {
    ...
    loadEquipLineTableData() {//环比数据
      ...
      // if(this.totalType=='mom'){
        selectFacilityEnergyHourList(queryParams).then((res) => {
          // console.log("上期", res);
          let lastAllData=JSON.parse(JSON.stringify(res.data))
          ...
          if(lastVal&&lastVal.Times&&lastVal.Times.length>0){
            let Times=JSON.parse(JSON.stringify(lastVal.Times))
            lastBarData=this.processTrendData(Times)
          }else{
            lastBarData=[]
          }

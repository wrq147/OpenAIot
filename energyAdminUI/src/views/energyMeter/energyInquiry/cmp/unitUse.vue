  ...
  <div>
      <div class="table_pie">
        <div class="meter_reading">
          <div class="meter_reading_li">
            <img src="@/assets/images/meter_icon.png" alt="">
            <div class="reading_li_right">
              <div class="reading_li_label">
                <span>起始表码值</span>
                <span class="unit">{{activeEngryUnit}}</span>
              </div>
              <div class="reading_li_value">
                <span>{{meterReadingStart}}</span>
              </div>
            </div>
          </div>
          <div class="meter_reading_li">
        ...
        <div class="table_con">
            <div class="table_ul" v-for="(ite,inx) in tableDataArray" :key="ite.type" :class="{'oddNumber':inx%2==0}">
              <div class="table_li first">
                <span>{{ite.title}}</span>
                <el-tooltip v-if="ite.tip" placement="right" popper-class="unit_tips" :content="ite.tipText">
                  <i class="zhongtaiiconfont zhongtai-icon-bangzhu" style="color: rgba(255, 255, 255, 0.6); font-size: 12px;margin-left:4px;"></i>
                </el-tooltip>
              </div>
              <div class="table_li">
        ...
        handler(val){
          if(val){
            ...
            if(this.unitSelect=='Unit'){
                ...
                this.meterReadingStart=newVal.InitVale
                this.meterReadingEnd=newVal.EndVale
            }else if(this.unitSelect=='LageUnit'){
                this.activeEngryUnit=newVal.LageUnit
                this.meterReadingStart=newVal.InitVale?Number((Number(newVal.InitVale)/10000).toFixed(6)):'-'
                this.meterReadingEnd=newVal.EndVale?Number((Number(newVal.EndVale)/10000).toFixed(6)):'-'
            }
            // console.log("设备统计",newVal);
            if(newVal&&newVal.Details&&newVal.Details.length>0){
              let Details=JSON.parse(JSON.stringify(newVal.Details))
              this.echartBarData=Details.map(row=>{

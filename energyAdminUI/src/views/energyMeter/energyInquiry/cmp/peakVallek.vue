  ...
  <div>
    <div class="table_pie">
      <div class="table_con">
          ...
          <div class="table_li">成本({{activeEngryUnit=='LageUnit'?'万元':'元'}})</div>
          <div class="table_li">标准煤({{activeEngryUnit=='LageUnit'?'tce':'kgce'}})</div>
          <div class="table_li">碳排放({{activeEngryUnit=='LageUnit'?'tCO₂e':'kgCO₂e'}})</div>
          </div>
    ...
    <strategy :timeType="timeType" :startTime="queryParams.beginDate" :endTime="queryParams.endDate" ref="strategy" :strategyList="strategyList" :factorName="factorName" :equipName="equipName"></strategy>
  </div>
</template>
  ...
  watch:{
    allData:{
        handler(newVal){
            ...
            this.echartBarData=newValArr.filter(row=>row.TimePeriod!='合计')

                ...
                this.$nextTick(() => {
                    if (this.facilityTreeList && this.facilityTreeList.length > 0) {
                        if(!this.isSetEquipVal){
                            ...
                            let currentKeyName= res.data[0].Id;
                            this.$refs['treeSelect'].setCurrentKey(this.currentKey);
                            this.$emit('getList', this.currentKey, false,res.data[0].FacilityName);
                        }else{
                            if(this.activeEquipObj){
                                this.currentKey=this.activeEquipObj.Id
                                this.$refs['treeSelect'].setCurrentKey(this.currentKey);
                                this.$emit('getList', this.currentKey, false,this.activeEquipObj.FacilityName);
                            }
                        }
            ...
            return nodes.map(node => {
                ...
                if (isLastLevel && node.Equipments && node.Equipments.length > 0) {
                    ...
                    if(this.activeEquipObj==null){
                        node.Equipments[0].FacilityName=node.Equipments[0].EquipmentName
        ...
        handleNodeClick(data) {
            if(!this.isSetEquipVal){
                if(data.Id!=this.currentKey){
                    ...
                    this.$emit('getList', this.currentKey, data.isEquipment,data.FacilityName);
                }else{
                    this.$refs['treeSelect'].setCurrentKey(this.currentKey);
                }
            }else{
                if(data.isEquipment&&data.Id!=this.currentKey){
                    this.currentKey = data.Id;
                    this.$emit('getList', this.currentKey, data.isEquipment,data.FacilityName);
                }else{
                    this.$refs['treeSelect'].setCurrentKey(this.currentKey);
        ...
        treeSearch() {
            ...
            this.$nextTick(() => {
                ...
                if (matchedNodes.length > 0) {
                    ...
                    this.$refs['treeSelect'].setCurrentKey(this.currentKey);
                    this.$emit('getList', this.currentKey,matchedNodes[0].FacilityName);
                }
            });
        },

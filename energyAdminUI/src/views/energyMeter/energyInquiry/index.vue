<template>
  <div style="padding: 10px 10px 10px 10px" id="big_con" class="energy_inquiry_con">
    <div class="energy_inquiry_left">
      <factorType @getList="getList" :isSetEquipVal="true"/>
    </div>
    <div class="energy_inquiry_right">
      ...
      <peak-vallek :timeType="dateVal.Type" :equipName="equipName" :factorName="factorName" v-if="dataTypeVal==2" ref="peakVallek" :queryParams="queryParams" :allData="allDataArray" :unitSelect="unitSelect" :tableConHeight="tableConHeight"></peak-vallek>
      <unit-use :dateVal="dateVal" v-if="dataTypeVal==1&&activeTabs=='statistics'" ref="unitUse" :queryParams="queryParams" :allData="useTotalInfo" :unitSelect="unitSelect" :tableConHeight="tableConHeight"></unit-use>
      <mom-yoy :totalType="activeTabs=='YoY'?'yoy':'mom'" :dateVal="dateVal" v-if="dataTypeVal==1&&activeTabs=='MoM'||dataTypeVal==1&&activeTabs=='YoY'" ref="momYoy" :queryParams="queryParams" :allData="useTotalInfo" :unitSelect="unitSelect" :tableConHeight="tableConHeight"></mom-yoy>
    </div>
  </div>
</template>
...
import factorType from '@/views/energyMeter/energyInquiry/cmp/factorType.vue';
import { selectFactorTypeOrg } from "@/api/energy/factorLibrary";
export default {
  name: "energyInquiryAdminUIIndex",
  ...
  methods: {
    ...
    loadLineTableData() {
      selectEnergyHourList(this.queryParams).then((res) => {
        // console.log("设备峰谷数据", res);
        this.allDataArray = res.data;
        this.useTotalInfo=res.data.find(row=>row.TimePeriod=='合计')
      });
    },
    ...
    async getList(equipmentId,isEquipment,equipName) {
      if(equipName){

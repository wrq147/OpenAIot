<template>
  <div>
    ...
    <div
      ...
      :style="{ 'min-height': 'calc(100vh - 194px' }"
        ...
        <div class="debugLeft">
          ...
          >
            <el-form-item>
              <el-select
                style="width: 130px"
                v-model="debugFrom.debugType"
                placeholder="请选择"
                @change="changeDebugType"
              >
                <el-option label="文本调试" value="文本调试"></el-option>
                <el-option
                  label="属性调试"
                  value="属性调试"
                  v-if="CanModbus == false"
                ></el-option>
          ...
          <div class="attrList">
            ...
            <el-row :gutter="10">
              ...
              <div
                v-else-if="debugFrom.debugType == '特殊消息'"
              ...
              >
               ...
               <div
                v-else-if="debugFrom.debugType == '事件消息'"
                style="padding-top: 15px;display: flex;flex-direction: column;align-items: center;"
              >
              <el-table
                :data="attrData"
                ...
                >
                  <template slot-scope="scope">
                    <el-button
                      ...
                      plain
                      @click="implementRowData(scope.row, scope.$index)"
                      >执行</el-button
                    >
                  </template>
                </el-table-column>
              ...
              >
                <el-table-column
                  v-if="debugFrom.debugType == 'Modbus调试'"
                  prop="Name"
                  label="名称"
                  align="center"
                ></el-table-column>
        ...
        <div class="debugRecord">
          ...
          <div class="record_list">
            <!-- <el-table
              ...
              :header-row-style="{'background-color':'#B5B5B5'}"
            >
              <el-table-column prop="name" fixed label="时间/日志类型"></el-table-column>
              <el-table-column prop="code" label="内容"></el-table-column>
            </el-table>-->
            <!-- <div style="background:#fff"> -->
            <mq-console
              ref="mq-console"
              :xkey=" debugFrom.debugDevice && deviceListMap.get(debugFrom.debugDevice) ? deviceListMap.get(debugFrom.debugDevice).DeviceId: ''"
              :isSubs="true"
...
export default {
  ...
  data() {
    return {
      ...
      recordData: [
        // { name: "地理位置", code: "Geolocation", type: "STRUCT" },
        // { name: "地理位置", code: "Geolocation", type: "STRUCT" },
        // { name: "地理位置", code: "Geolocation", type: "STRUCT" },
        // { name: "地理位置", code: "Geolocation", type: "STRUCT" },
        // { name: "地理位置", code: "Geolocation", type: "STRUCT" },
  ...
  methods: {
    ...
    choiceDeviceVal(val) {
      ...
      if(val==null||val==""){
        if(this.debugFrom.debugDevice){
          this.$refs["mq-console"].disconnect(this.debugFrom.debugDevice,()=>{});
          this.debugFrom.debugDevice=val
        }
        return;
      }else{
        valDeviceId=this.deviceListMap.get(val).DeviceId
      }
      if(this.debugFrom.debugDevice!=null&&this.debugFrom.debugDevice!=''){
        let beforeDevId=this.deviceListMap.get(this.debugFrom.debugDevice).DeviceId
        this.$refs["mq-console"].disconnect(beforeDevId,()=>{
          this.$refs["mq-console"].connect(valDeviceId);
          this.debugFrom.debugDevice=val
        });
      }else{
        this.$refs["mq-console"].connect(valDeviceId);
    ...
    implementRowData(row, indexRow) {
      ...
      if (this.debugFrom.debugType == "Modbus调试") {
        ...
        this.$set(this.implementParamsForm,"inputsData",JSON.parse(JSON.stringify(lis)));

        this.resetForm("implementParamsForm");
        this.implementParams = true;
      }else if(this.debugFrom.debugType == "事件消息"){
        implementEvent({DeviceId: this.debugFrom.debugDevice,EventId:row.code,Inputs:{}}).then((res) => {
          this.$modal.msgSuccess("发送成功");
        });
      }
    },
    ...
    initClassMap(node) {
      for (let idx = 0; idx < node.length; idx++) {
        ...
        this.deviceListMap.set(node[idx].Id, curnode);
        if (

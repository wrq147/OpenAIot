      ...
      <div>
        <div class="header" style="border-top: none; padding: 0; border-bottom: 1px solid #dadada">
          <el-menu :default-active="activeDevice" active-text-color="#409eff" class="el-menu-demo shejiqi" mode="horizontal">
            <el-menu-item index="instanceInformation" @click="deviceSelect('instanceInformation')" :key="'menubas'+menukey">实例信息</el-menu-item>
            <el-menu-item v-if="ishasIot" index="runningState" @click="deviceSelect('runningState')" :key="'menustatus'+menukey">运行状态</el-menu-item>
            <el-menu-item index="equipmentLocation" @click="deviceSelect('equipmentLocation')" v-if="positionInfo && positionInfo.enable" :key="'menupos'+menukey">设备位置</el-menu-item>
            <el-menu-item index="warning" @click="deviceSelect('warning')" :key="'menuwarn'+menukey">报警信息</el-menu-item>
            <el-menu-item index="func" @click="deviceSelect('func')" v-if="ishasIot" :key="'menufunc'+menukey">设备功能</el-menu-item>
            <el-menu-item index="abnormalData" @click="deviceSelect('abnormalData')" v-if="ishasIot" :key="'menuabnor'+menukey">异常数据</el-menu-item>
            <el-menu-item index="devicePlane" @click="deviceSelect('devicePlane')" v-if="isShowPlane" :key="'menuplane'+menukey">设备计划</el-menu-item>
            <el-menu-item index="onLineDebug" @click="deviceSelect('onLineDebug')" v-if="productInfos.Status == '0'&&isCheckPermi(['/IoTService/IotProduct/ListPage'])" :key="'menudebug'+menukey">在线调试</el-menu-item>
    ...
    init(lat, lng, dom = "allmaptt") {
      ...
      this.infoWindow = new AMap.InfoWindow({
        ...
        content:
          ...
          "</div>" +
          '<div align="left" class="content">设备状态：' +
          line +
          "</div>" +

<template>
  <div :class="{'node': true, 'node-error-state': showError}">
    <div :class="{'node-body': true, 'error': showError}">
      ...
      <div class="node-body-main">
        <div class="node-body-main-header">
          ...
          <span class="level" style="margin-left: 10px;">优先级{{ level }}</span>
          <span class="option" style="margin-left: 10px;">
            <el-tooltip content="复制条件" placement="top">
    ...
    <div class="node-footer" v-if="size>1">
      <div class="btn">
...
export default {
      ...
      groups.forEach(group => {
        ...
        group.conditions.forEach(subCondition => {
          ...
          switch (subCondition.valueType) {
            case ValueType2.date:
              if(subCondition.compare=="before"){
                subConditionStr=`${this.codiname(subCondition.gname,subCondition.title)}在${subCondition.value==null||subCondition.value==""?'':this.parseTime(subCondition.value)}之前`;
              }
              else{
                subConditionStr=`${this.codiname(subCondition.gname,subCondition.title)}在${subCondition.value==null||subCondition.value==""?'':this.parseTime(subCondition.value)}之后`;
              }
              break;
            case ValueType2.enum:
              {
                if(subCondition.codeIsArrary==true){
                  switch (subCondition.compare) {
                    case '=':
                      subConditionStr = `${this.codiname(subCondition.gname,subCondition.title)}包含${this.getDefault(subCondition.value, ' ?')}`
                      break;
                    case '!=':
                      subConditionStr =  `${this.codiname(subCondition.gname,subCondition.title)}不包含${this.getDefault(subCondition.value, ' ?')}`
                      break;
                    default:
                      subConditionStr = '';
                      break;
  ...
  methods: {
    ...
    addNode() {
      ...
      console.log("点击按钮时节点信息", this.selectedNode);
    },
    getDefault(val, df) {
    ...
    getOrdinaryConditionContent(subCondition) {
      switch (subCondition.compare) {
        case 'IN':
          return `${this.codiname(subCondition.gname,subCondition.title)}包含${subCondition.value}`
        default:
          return `${this.codiname(subCondition.gname,subCondition.title)}${subCondition.compare}${this.getDefault(subCondition.value, ' ?')}`
      }
    },
    //校验数据配置的合法性
    validate(err) {
        ...
        for (let i = 0; i < props.groups.length; i++) {
          if (props.groups[i].cids.length === 0){
            ...
            for (let ci = 0; ci < conditions.length; ci++) {
              ...
              if (this.showError){
                this.errorInfo = `请完善条件组${this.groupNames[i]}内的${this.codiname(subc.gname,subc.title)}条件`
                err.push(`条件 ${this.config.name} 条件组${this.groupNames[i]}内${this.codiname(subc.gname,subc.title)}条件未完善`)
                return false
              }
            }
          }

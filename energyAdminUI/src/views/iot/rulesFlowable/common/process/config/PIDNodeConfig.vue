  ...
  <div>
    <el-form label-width="110px">
      <el-form-item label="参考设备类型：">
        <el-select style="width:100%" :value="devType" placeholder="请选择参考设备类型" @change="choiceTargetType">
          <el-option label="当前设备" :value="0" v-if="enableCur"></el-option>
          <el-option label="选择设备" :value="1"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="参考设备：" v-if="devType == 1">
        <el-select style="width:100%" v-model="config.refdevid" clearable filterable remote reserve-keyword :remote-method="remoteMethod"
          @clear="remoteMethod('')" :loading="loading" placeholder="请选择参考设备" @change="devChange">
      ...
      <el-form-item label="设备属性：" v-if="config.TargetType==0||config.TargetType==1&&config.refdevid">
      ...
      <el-form-item label="目标值参数：" v-if="config.TargetType==0||config.TargetType==1&&config.refdevid">
      ...
      <el-form-item label="比例参数：" v-if="config.TargetType==0||config.TargetType==1&&config.refdevid">
      ...
      <el-form-item label="积分时间：" v-if="config.TargetType==0||config.TargetType==1&&config.refdevid">
      ...
      <el-form-item label="微分时间：" v-if="config.TargetType==0||config.TargetType==1&&config.refdevid">
    ...
    <el-dialog :title="sceTitle" append-to-body :close-on-click-modal="false" :visible.sync="scheOpen" width="740px">
      <el-form label-width="150px" v-if="editSet != null">
        <el-form-item label="控制设备类型：">
          <el-select style="width:100%" v-model="editSet.TargetType" placeholder="请选择控制设备类型" @change="choiceTargetType2">
            <el-option label="当前设备" :value="0" v-if="enableCur"></el-option>
            <el-option label="选择设备" :value="1"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="控制设备：" v-if="editSet.TargetType == 1">
          <el-select style="width:100%" v-model="editSet.id" clearable filterable remote reserve-keyword :remote-method="remoteMethod2"
            @clear="remoteMethod2('')" :loading="loading" placeholder="请选择控制设备" @change="devChange2">
...
export default {
  ...
  computed: {
    devType(){
      if(this.config.refdevid==null||this.config.refdevid==""){
        return 0;
      }
      else{
        return 1;
    ...
    ParamList() {
      ...
      console.log(httpss,'httpsshttpss');
      if (httpss != null) {
        return httpss;
      }
      else {
    ...
    defaultProductId(){
      let TriggerList=this.setup.TriggerList
    ...
    enableCur() {
      ...
      if (this.$store.state.rulesFlowable.selectProductInfo != null&&this.$store.state.rulesFlowable.rulesDesign.TriggerWay==1) {
        return true;
      }
      else {
        return false;
  ...
  methods: {
    ...
    addDevController(){
      this.config.DevControllerItem.push({TargetType:null,id:'',maxval:'',minval:'',code:'',codeval:'' });
    },
    ...
    devChange() {
      ...
      if(this.config.TargetType!==null&&this.config.TargetType!==''&&this.config.TargetType!==undefined){

    ...
    <div style="display: flex;flex-direction: row;">
      ...
      <div class="info-right">
        <div style="display: flex;flex-direction: row;">
          ...
          <span class="del-btn" @click="clearHumerText"><i class="el-icon-delete" style="margin-right: 5px;"></i>清除</span>
        </div>
        <div :class="{ 'othdis': customDisabled }">
          ...
          <div>
            <el-select :disabled="customDisabled" :clearable="true" @clear="devChange('')" size="small" v-model="filterId"
              filterable remote reserve-keyword placeholder="请输入搜索关键词" :remote-method="remoteMethod"
          ...
          <div>
            <select size="3" class="in-list" :disabled="customDisabled">
              ...
              <option class="in-item" v-show="filterId==''" :value="getCurPropVal(opx.code)" v-for="opx in propItems" :key="'#xxpp'+opx.code"
                @click="addItem($event, 'custom', '【属】' + opx.name, getCurPropVal(opx.code), customDisabled)">
                {{ '【属】'+opx.name }}
              </option>
              <option class="in-item" v-show="filterId==''" :value="getCurTagVal(tpx.code)" v-for="tpx in tagItems" :key="'#xx'+tpx.code"
          ...
          productInfo({ id: this.filterDevice.ProductId }).then(rsp => {
            if (rsp.code == 0) {
              ...
              if (jsonLis.properties) {
                this.DevPropList = jsonLis.properties.filter(x=>x.option.type!='geo');
                this.DevTagList = jsonLis.tags.filter(x=>x.option.type!='geo');
              }
            }
          });
        }

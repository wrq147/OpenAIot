<template>
  <div>
    <div style="padding:10px 10px 50px 10px" id="big_con">
            ...
            <el-steps direction="vertical" :active="addActive" finish-status="success">
              <el-step title="请选择你想创建的产品分类">
                <div slot="title">
                  ...
                  <span style="color: #0f73e6;font-size: 12px;margin-left: 30px;cursor: pointer;" @click="reselectClass" v-if="selectedClass.Id != null">重新选择</span>
                </div>
                  ...
                  <el-card v-else class="class_box_card" v-loading="cardLoading">
                    <div class="class_item_con">
                      ...
                      <div style="width: 100%" v-else>
                        <el-empty>
                          <el-link type="primary" @click="$router.push('/iot/physicalModel/productClass')">点击此处创建</el-link>
                        </el-empty>
                      </div>
                    </div>
                  </el-card>
              ...
              <el-step title="请选择设备接入方式">
                <div slot="title">
                  ...
                  <span style="color: #0f73e6;font-size: 12px;margin-left: 30px;cursor: pointer;" @click="reselecWorkWay" v-if="networkWay != null">重新选择</span>
                </div>
                <div slot="description" style="padding: 20px 10px 32px" v-if="(networkWay && networkWay.Code) || addActive == 1">
                  <div class="selected_class" v-if="(networkWay &&networkWay.Code && networkWay.Code == 'mqtt_modbus' &&addData.interScripts) ||(networkWay &&networkWay.Code &&networkWay.Code != 'mqtt_modbus')" style="justify-content: flex-start">
                    ...
                    <div class="workWay_con">
                      <div class="workWay_item_con" v-for="ites in workWayList" :key="ites.Code">
                        <div class="workWay_item" @click="choiceWorkWay(ites)">
                          {{ ites.Name }}<div class="workWaySelectDiv" v-if="networkWay && networkWay.Code && networkWay.Code == ites.Code"></div>
                        </div>
                    ...
                    <el-row v-if="networkWay && networkWay.Code == 'mqtt_modbus'" style="margin-top: 20px">
                      <el-col :span="24">
                        <el-form-item label="DTU模板" style="margin-bottom: 0">
                          <el-select v-model="addData.interScripts" filterable remote reserve-keyword placeholder="请输入关脚本模板名称" :remote-method="remoteMethod" :loading="optionLoading" clearable @change="choiceScriptVal">
                            <el-option v-for="item in scriptOption" :key="item.Id" :label="item.Name" :value="item.Id"></el-option>
                          </el-select>
                        </el-form-item>
                      </el-col>
                    </el-row>
                    ...
                    <el-row>
                      <el-col :span="24">
                        <el-form-item label="产品名称" prop="name">
                          <el-input v-model="addData.name" @input.native="inputName" placeholder="请输入产品名称" maxlength="20"/>
                        </el-form-item>
                      </el-col>
                    </el-row>

                    <el-form v-if="networkWay.Code != ''" ref="StorageConfig" :model="StorageConfig" :rules="StorageConfigRules" label-width="80px" class="storage_config">
                      <el-row>
                        <el-col :span="24">
                          <el-form-item label="存储方式" prop="enable">
                            <el-select v-model="StorageConfig.enable" placeholder="请选择存储方式">
                              <el-option v-for="item in StorageConfigList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                            </el-select>
                          </el-form-item>
                        </el-col>
                      </el-row>
                      <el-row>
                        <el-col :span="24">
                          <el-form-item label="存储组织" prop="org" v-if="StorageConfig.enable == 1">
                            <el-input v-model="StorageConfig.org" placeholder="请输入存储组织"></el-input>
                          </el-form-item>
                        </el-col>
                      </el-row>
                      <el-row>
                      ...
                      <el-col :span="24">
                        <el-form-item label="通讯方式" prop="physicsWay">
                          <el-select v-model="addData.physicsWay" multiple :multiple-limit="4" placeholder="请选择通讯方式">
                            <el-option v-for="item in physicsWayList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                          </el-select>
                        </el-form-item>
                      </el-col>
                    </el-row>
...
export default {
  ...
  methods: {
    ...
    addPro() {
      ...
      let StorageConfig = {};
      if (this.StorageConfig.enable == 0) {
        StorageConfig = {
          enable: this.StorageConfig.enable,
        };
      } else if (this.StorageConfig.enable == 1) {

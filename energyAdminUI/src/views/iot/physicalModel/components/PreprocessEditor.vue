    ...
    <div style="display: flex;flex-direction: row;">
      ...
      <div class="info-right">
        <div style="display: flex;flex-direction: row;">
          ...
          <div>
            <select size="9" class="in-list" :disabled="customDisabled">
              <option class="in-item" value="toFloat(data)" @click="addItem($event, 'data', '浮点型原值', 'toFloat(data)', dataDisabled)">原值转浮点数</option>
              <option class="in-item" value="byteTo(data)" @click="addItem($event, 'data', '按位转字符串', 'byteTo(data)', dataDisabled)">按位转字符串</option>
              <option class="in-item" value="byteAllTo(data,\'\',\'\')" @click="addItem($event, 'data', '按位全转字符串', 'byteAllTo(data,\'\',\'\')', dataDisabled)">按位全转字符串</option>
              <option class="in-item" :value="getCurPropVal(opx.code)" v-for="opx in attrTableData"
                :key="'#xxpp' + opx.code"
...
export default {
  ...
  methods: {
    ...
    isExpress(str) {
      try {
        let callFunction = eval("(data,toFloat,byteTo,byteAllTo,prop)=>" + str);
        let rs = (callFunction)(1, function (k) { return 1.1; }, function () { return ""; }, function () { return ""; }, function (k) { return 1; });
    ...
    async parsingFormula(formulaStr) {
      ...
      for (let i = 0; i < formulaStr.length; i++) {
        let cc = formulaStr[i];
        if (funcStr == "" && symStr == "") {
          if (this.Num.some(x => x.value == cc)) {
            this.backFormulaStrArray.push({ type: "num", name: cc, value: cc });
          }
          else if (cc == "&" || cc == "|") {
            symStr += cc;
          }
          else if (this.Symbol.some(x => x.value == cc)) {
            this.backFormulaStrArray.push({ type: "symbol", name: cc, value: cc });
          }
          else {
            funcStr += cc;
          }
        }
        else if (symStr.length > 0 && (cc == "&" || cc == "|")) {
          symStr += cc;
          if (symStr == "&&") {

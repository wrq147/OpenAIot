<template>
  <div>
    <el-drawer title="规则匹配" :visible.sync="matchesDrawer" direction="rtl" @close="closeMatchesDrawer" size="900px" :destroy-on-close="true">
      ...
      <div style="padding-left: 20px; color: #97999c" class="matches_title">
        ...
        <div class="tongji_con">
          <div class="tongji_li" v-for="it in NumRegisterList" :key="it.label+'_sum_'+it.value">
            ...
            <span class="num">{{countByteNum(it.value)}}</span>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        style="text-align: center; margin-top: 40p; padding-bottom: 20px"
      >
        <el-button @click="closeMatchesDrawer">取 消</el-button>
        <el-button
          type="primary"
          @click="joinMatchesForm"
          :loading="matchesLoading"
          >{{ matchesLoading ? "提交中 ..." : "保 存" }}</el-button
        >
      </div>
    </el-drawer>
  </div>
    ...
    joinMatchesForm() {
      ...
      this.$refs["matchesForm"].validate((val1) => {
        ...
        for (let i = 0; i < this.matchesItemsForm.length; i++) {
          succNum += 1;
          if (
            succNum == this.matchesItemsForm.length &&
            i == this.matchesItemsForm.length - 1
          ) {
            if (this.activeParamsLine > -1) {
              this.matchesForm.Items = JSON.parse(
                JSON.stringify(this.matchesItemsForm)
              );

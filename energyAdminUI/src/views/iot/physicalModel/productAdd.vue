  ...
  <div style="position: relative;">
    <div style="position: absolute;top: 0;left: 0;width:100%;z-index:1;">
      ...
      <div class="header">
        <el-menu :default-active="activeSelect" active-text-color="#409eff" class="el-menu-demo shejiqi"
          ...
          <el-menu-item index="warning" @click="to('warning')">报警信息</el-menu-item>
        ...
        <div class="elbiaoge_elform" :style="{ 'min-height': 'calc(100vh - 194px' }" v-if="activeSelect == 'physicalModel'">
          <div>
            ...
            <div style="padding:20px;">
              <div>
                <el-row :gutter="10" class="mb8 button_row" v-if="activeDefinition != 'modbus'&&activeDefinition != 'firmwareFiles'">
                  <div v-if="activeDefinition != 'expands'">
                    <el-col :span="1.5">
                      <el-button type="primary" plain @click="openAttrDrawer">
                ...
                <firmwareFiles :productInfos="productInfos" @changeSrc="filesUnload" v-if="activeDefinition == 'firmwareFiles'"></firmwareFiles>
              </div>
            </div>
          </div>
        </div>
      ...
      <el-form class="attrFrom_con" label-width="100px" :rules="attrRules" ref="attrFrom"
        ...
        <el-form-item label="标识符" prop="code" v-if="activeDefinition != 'modbus'||isModbusAddAttribute">
          <el-input v-if="activeDefinition == 'attribute'||isModbusAddAttribute" v-model="attrFrom.code" placeholder="请输入标识符"
            ...
            :disabled="isEditCode && attrFrom.code != ''" />
          ...
          <el-input v-if="activeDefinition == 'function'" v-model="funcFrom.code" placeholder="请输入标识符"
            ...
            :disabled="isEditCode && funcFrom.code != ''" />
          <el-input v-if="activeDefinition == 'event'" v-model="eventFrom.code" placeholder="请输入标识符"
            ...
            :disabled="isEditCode && eventFrom.code != ''" />
        ...
        <el-form-item label="启用条件" v-if="activeDefinition == 'function'">
          ...
          <div class="dlg-param-bg" v-if="funcFrom.conditions!=null&&funcFrom.conditions.length > 0">
            <div class="paramrow" v-for="(ccitem, cidx) in funcFrom.conditions" :key="cidx">
              <span style="margin-right: 10px;width:45px;">条件{{ cidx + 1 }}</span>
        ...
        <el-form-item label="条件组合" v-if="activeDefinition == 'function'&&funcFrom.conditions!=null&&funcFrom.conditions.length > 1">
          <div class="dlg-gg-row">
            <div style="margin-bottom: 10px;" v-for="(ggitem, ggidx) in LoopCount" :key="ggidx">
              ...
              <el-select style="width: 80px;margin-right: 10px;" v-model="funcFrom.groups[ggidx]"
                @change="chgGroup($event, ggidx)">
                <el-option label="且" value="&"></el-option>
                <el-option label="或" value="|"></el-option>
              </el-select>
              <span v-if="ggidx == (LoopCount.length - 1)" style="margin-right: 10px;">条件{{ funcFrom.conditions.length
                }}</span>
            </div>
          </div>
        </el-form-item>
        ...
        <type-form ref="typeFormAssembly" v-if="activeDefinition == 'attribute'||isModbusAddAttribute || activeDefinition == 'expands'"
          :activeDefinition="activeDefinition" :enumKeyList="enumKeyList" :paramsForm="paramsForm" :attrTableData="FilterAttr"
          @changeMapcode="changeMapcode" @setCurType="setCurType" :isModbusAddAttribute="isModbusAddAttribute"></type-form>
        <el-alert  v-if="activeDefinition == 'expands'&&expandsForm.code=='state'"
    title="只有设备运行状态在枚举值里时，属性才能变更设备的运行状态"
    type="warning"
    ...
    <el-drawer title="添加参数" ref="funcDra" :wrapperClosable="false" :visible.sync="funcDrawer" @close="closeParamDrawer"
      ...
      <div class="demo-drawer__footer" style="text-align: center;margin-top:40p;padding-bottom:20px">
        ...
        <el-button type="primary" @click="joinParams" :loading="paramsLoading">{{ paramsLoading ? '提交中 ...' : '确 定'
          }}</el-button>
      </div>
    </el-drawer>
    <el-dialog
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="24" :xs="24">
          <!-- <div class="from_con" id="from_con"> -->
            ...
            >
              <el-form-item label="搜索关键词" prop="SearchKey">
                <el-input
                  class="set_radius"
                  v-model="scriptQueryParams.SearchKey"
                  placeholder="请输入关键字"
                  clearable
              ...
              <el-form-item class="submit_button_con">
                <el-button icon="el-icon-refresh" @click="resetScriptQuery"
                  >重置</el-button
                >
                <el-button
                  type="primary"
                  icon="el-icon-search"
                  @click="handleScriptQuery"
                  >搜索</el-button
                >
              </el-form-item>
            </el-form>
          ...
          <el-row :gutter="10" justify="start">
            ...
            >
              <div class="script_li" @click="choiceOneTemplete(item)">
                ...
                <div
                  class="tags_info"
                  v-if="scriptSelected.Id&&scriptSelected.Id==item.Id"
                ></div>
                <div>
                  <el-input
                    type="textarea"
          ...
          <pagination
            v-show="scriptTotal > 0"
            :total="scriptTotal"
            :page.sync="scriptQueryParams.pageNum"
            :limit.sync="scriptQueryParams.pageSize"
            :pageSizes="scriptPageSizes"
    ...
    <copy-item ref="cpyRef" @ok="confirmCopy">
    </copy-item>
...
export default {
  ...
  computed: {
    FilterAttr(){
      if(this.attrFrom.option.type=='int'||this.attrFrom.option.type=='float'){
        return this.attrTableData.filter(item =>(item.option.type=='float'||item.option.type=='int'||item.option.type=='date')&&item.code!=this.attrFrom.code);
      }
      else if(this.attrFrom.option.type=='boolean'){
        return this.attrTableData.filter(item =>item.option.type=='boolean'&&item.code!=this.attrFrom.code);
      }
      else{
        return this.attrTableData.filter(item =>(item.option.type=='string'||item.option.type=='enum')&&item.code!=this.attrFrom.code);
      }
    },
    modbuslist() {
    ...
    LoopCount() {
      if (this.funcFrom == null) return [];
      if (this.funcFrom.conditions.length > 1) {
        if (this.funcFrom.groups == null) {
          this.funcFrom.groups = [];
        }

        let ll = this.funcFrom.conditions.length - 1;
        for (let i = 0; i < ll; i++) {
          if (i >= this.funcFrom.groups.length) {
            this.funcFrom.groups[i] = "&";
          }
        }
        if (this.funcFrom.groups.length > ll) {
          this.funcFrom.groups.splice(ll);
        }
        return this.funcFrom.groups;
      }
      else {
        return [];
      }
  ...
  methods: {
    ...
    setCurType(curType) {
      console.info(this.curType);
      console.info(curType)

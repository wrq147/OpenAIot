    ...
    <el-dialog :title="title" :close-on-click-modal="false" :visible.sync="open" v-loading="allloading" width="1050px" top="2vh">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
          ...
          <el-col :span="16">
            <el-form-item label="物流单号" prop="ExpressNumber">
              <el-input placeholder="请输入物流单号" v-model="form.ExpressNumber" @change="expressSelected">
                <el-select v-if="form.ExpressNumber != ''" style="width: 130px" v-model="form.ExpressCompany" slot="prepend" placeholder="请选择物流公司">
                  <el-option v-for="kditem in kdcompanys" :key="kditem.Code" :label="kditem.Name" :value="kditem.Code"></el-option>
                </el-select>
              </el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="所出仓库" prop="FromHouseId">
              <div style="display: flex; align-items: center">
                <el-input class="houseipt" v-model="form.FromHouseName" readonly placeholder="请选择所出仓库" @focus="openHouseDialog">
          ...
          <el-col :span="12">
            <el-form-item label="目标仓库" prop="ToHouseId">
              <div style="display: flex; align-items: center">
                <el-input class="houseipt" v-model="form.ToHouseName" readonly placeholder="请选择目标仓库" @focus="openTargetHouseDialog">
                  <i slot="suffix" @click="onClearTo" v-if="form.ToHouseId != null" class="el-icon-circle-close"
                    style="font-size: 22px;cursor: pointer;vertical-align: middle;"></i>
                </el-input>
              </div>
            </el-form-item>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="备注说明" prop="Remark">
              <el-input type="textarea" :rows="2" placeholder="请输入备注说明" v-model="form.Remark" style="width: 80%"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <div>
      ...
      <el-dialog width="960px" title="请选择导入的物品" :visible.sync="wupinOpen" :close-on-click-modal="false" append-to-body>
        <el-form :model="wupinQuery" :inline="true" ref="wupinForm" style="display: flex; justify-content: space-between">
          <div>
            ...
            <el-form-item label="创建日期">
              <el-date-picker class="form_input_style" v-model="wupinDateRange" style="width: 232px" value-format="yyyy-MM-dd" type="daterange"
        ...
        <el-table ref="wupinTable" :data="wupinList" tooltip-effect="dark" v-loading="wupinLoading" style="width: 100%" @selection-change="onWupinChange">
          ...
          <el-table-column prop="TargetType" align="center" label="存储类型" width="220">
            <template slot-scope="scope">
              <div>{{ scope.row.TargetType == 1 ? "成品" : "半成品" }}</div>
            </template>
          </el-table-column>
        </el-table>

        <pagination v-show="wupinTotal > 0" :total="wupinTotal" :page.sync="wupinQuery.pageNum" :limit.sync="wupinQuery.pageSize" @pagination="loadWupinList"/>
        <div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    async openDialog(id, fromHsId) {
      ...
      this.$nextTick(async () => {
        ...
        if (this.leaveId == null) {
          ...
          this.form.ToHouseId = undefined;
          this.form.ToHouseName = "";
          this.form.Remark = "";
          ...
          if (fromHsId != null) {
            let hsInfo = await houseInfo(fromHsId);
          ...
          this.form.ToHouseId = leaveInfo.data.ToHouseId;
          this.form.ToHouseName = leaveInfo.data.ToHouseName;
    ...
    async handleCurrentChange(val) {
      if (this.dlgType == "from") {
        ...
        this.form.ToHouseName = val.StoreName;
        this.form.ToHouseId = val.Id;
      }
    },
    loadWupinList() {
    ...
    submitForm(st) {
      this.$refs["form"].validate(async (valid) => {
        if (valid) {
          ...
          if (this.form.FromHouseId == this.form.ToHouseId) {
            this.$message.error("请选择不同的所出仓库和目标仓库！");
            return;
          }

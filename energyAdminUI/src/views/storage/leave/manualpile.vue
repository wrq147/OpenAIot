  ...
  <div style="padding: 20px 20px 0 20px; height: 100%" class="manualpage">
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-row v-if="!isApplyLeave">
          <el-col :span="16">
            <el-form-item label="物流单号" prop="ExpressNumber">
              <el-input
                placeholder="请输入物流单号"
                v-model="form.ExpressNumber"
                @change="expressSelected"
              >
                <el-select
                  v-if="form.ExpressNumber != ''"
                  style="width: 130px"
        ...
        <el-row v-if="!isApplyLeave&&form.ExpressCompany == 'shunfeng'">
          <el-col :span="16">
            <el-form-item label="联系电话" prop="ExpressPhone">
              <el-input
                placeholder="请输入物流单上的联系电话"
                type="tel"
        ...
        <el-row>
          <el-col :span="!isApplyLeave?24:12">
            <el-form-item label="所出仓库" prop="FromHouseId">
              <div style="display: flex; align-items: center">
                <el-input class="houseipt" v-model="form.FromHouseName"
                  readonly placeholder="请选择所出仓库" @focus="openHouseDialog" :disabled="isApplyLeave">
                  <i
                    slot="suffix"
                    @click="onClear"
                    v-if="form.FromHouseId != null"
          ...
          <el-col :span="12" v-if="!isApplyLeave">
            <el-form-item label="目标客户" prop="ToOrgId">
              <div style="display: flex; align-items: center">
                <el-input
                  class="houseipt"
                  v-model="form.ToName"
          ...
          <div class="items-title">
            ...
            <div>
              <el-row :gutter="15" type="flex" justify="end">
                <el-col :span="1.5" v-if="!isApplyLeave">
                  <el-button
                    type="warning"
                    size="mini"
                    plain
          ...
          <el-table :data="form.List" stripe style="width: 100%">
            ...
            <el-table-column align="center" label="数量" width="140">
              <template slot-scope="scope">
                ...
                <el-input-number v-else v-model="scope.row.Quantity" :min="1" :max="9999" size="mini" :disabled="isApplyLeave"></el-input-number>
              </template>
            </el-table-column>
            <el-table-column align="center" label="单价（元）" width="140" v-if="!isApplyLeave">
              <template slot-scope="scope">
                <el-input-number
            ...
            <el-table-column prop="address" align="center" label="操作" width="110" v-if="!isApplyLeave">
              <template slot-scope="scope">
      ...
      >
        ...
        >
          <div>
            <el-form-item label="搜索关键字" prop="Key">
              <el-input
                v-model="wupinQuery.Key"
                placeholder="搜索物品名称或物品编号"
                clearable
              ></el-input>
            </el-form-item>
        ...
        <el-table
          ...
          :data="wupinList"
          tooltip-effect="dark"
          v-loading="wupinLoading"
          style="width: 100%"
          @selection-change="onWupinChange"
          @row-click="clickWupinRow"
          @select="wupinBoxSelect"
          row-key="TargetId"
        >
        ...
        <pagination
          v-show="wupinTotal > 0"
          :total="wupinTotal"
          :page.sync="wupinQuery.pageNum"
          :limit.sync="wupinQuery.pageSize"
          @pagination="loadWupinList"
      ...
      <KfSelecter ref="kfDlg" @ok="onTargetChange"></KfSelecter>
...
export default {
  name: "ManualPile",
  components: { AddEmbed, KfSelecter, HouseSelecter },
  data() {
    return {
      title: "",
      kdcompanys: [],
      ...
      rules: {
        ...
        ToOrgId: [
          { required: true, message: "目标客户不能为空", trigger: "change" },
        ],
        FromHouseId: [
  ...
  methods: {
   ...
   async applyLeaveDialog(row){
      //申请出库单出库
      let info=await ApplyInfo(row.Id)
      ...
      this.form.ToOrgId = 0;
      this.form.ToName = "";
      this.form.sourceEnterId=row.Id
      this.form.List = info.data.List.map(rw=>{
        rw.Price=0
        return rw
      });
      // console.log(this.form.List,'物品列表');
      ...
      this.$nextTick(async () => {
        ...
        if (this.leaveId == null) {
          ...
          if (fromHsId != null) {
            ...
            if (hslist.length > 0) {
              this.form.FromHouseName = hslist[0].StoreName;
              this.form.FromHouseId = hslist[0].Id;
          ...
          console.log(leaveInfo,'FromHouseIdFromHouseId');
          this.form.StockNumber = leaveInfo.data.StockNumber;
          this.form.FromHouseName = leaveInfo.data.ToHouseName;
          ...
          if(leaveInfo.data.LeaveMethod==3){
            this.isApplyLeave=true//出库申请跳转致出库
            this.form.sourceEnterId=leaveInfo.data.SourceEnterId
            this.form.FromHouseName = leaveInfo.data.FromHouseName;
            console.log("出库物品",this.form.List);
          }
        }
    ...
    onTargetChange(val) {
      this.form.ToOrgId = val.OrgId;
      this.form.ToName = val.OrgName;
    },
    submitForm(st) {

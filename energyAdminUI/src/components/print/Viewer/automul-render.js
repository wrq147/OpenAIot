 ...
 * ╩╚═╚═╝ ╩ ╩╩ ╩╝╚╝╚═╝
 */
import { CONFIG } from '@/components/print/Viewer/viewer-constant'
import generateID from '@/components/print/js/generateID'
import {
    StyledCircle,
    StyledLine,
    StyledRect,
    ...
    StyledImage,
    StyledQrcode
} from '@/components/print/PageComponents/style'
import { RenderUtil } from '@/components/print/Viewer/render-util'
import Vue from 'vue'
import { AutoSplitText } from '@/components/print/Viewer/auto-split-text'
import { AutoTable } from '@/components/print/Viewer/auto-table'
...
export class AutoRender {
    ...
    async renderRoySimpleTable(
      ...
      }px;table-layout: auto;border-collapse: separate;border-spacing: 2px;background-color: #000000;'>${table.html}</table>`
    ...
    async renderRoyQrcode(element, zIndex = 0) {
        const { style } = element
        let newElement = this.createNewElementWithStyledComponent(element, zIndex)
        let img = document.createElement('img')
        const codeSrc = await RenderUtil.createCode(
            element,
            this.dataSet,
        )
        img.src = codeSrc

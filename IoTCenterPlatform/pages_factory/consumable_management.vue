<template>
	<view>
		<top :title="topTitle" leftText="Back" leftWidth="120rpx" rightWidth="60rpx" :isleftBack="true"
			leftIcon="icon-fanhui" backgroundColor="#161A26" rightIcon="icon-tianjia" @clickRight="consumableAdd">
		</top>
		...
		<view class="device_list_con">
			...
			<view v-if="isSelect&&isMulSelect">
				...
				<button class="submit_button" @click="confirmSelected">
					Confirm({{selectConsumable.length}})
				</button>
			</view>
...
<script>
	...
	export default {
		data() {
			return {
				topTitle: 'Consumable Management',
				querydata: {
					pageNum: 1,
					pageSize: 10,
				},
		...
		methods: {
			...
			getList() {
				...
				partsList(this.querydata).then(response => {
					console.log(response, 'response耗材管理');

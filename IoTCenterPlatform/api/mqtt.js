...
export function getClientId1() {
	var requestTask = uni.request({
		url: serverUrl.getServerUrl() + '/MqttService/User/ClientId',
		method: "GET",
		header: {
	...
	console.log("requestTask", requestTask, requestTask._xhr, requestTask._xhr.XMLHttpRequest);
	let req = requestTask._xhr
	console.log(req, "reqreqreqreq", req.status);
	if (req.status === 200) {
...
export function asyncClientIdOperation(oldOptions) {
  return new Promise(function(resolve, reject) {
    ...
    setTimeout(async function() {
      ...
      const newOptions = Object.assign({}, oldOptions, { clientId: newClientId.data });
      resolve(newOptions);

<template>
	<view id="CreateCustomer">
		<top leftIcon="icon-fanhui" :isleftBack="true" leftText="Back" backgroundColor="#161A26" title="Create clue"
		...
		methods: {
			handEditForm() {
				...
				this.$refs.form.validate().then((res) => {
					...
					// return
					ClueEdit(this.formData).then((data) => {
						if (data.code == 0) {
							uni.showToast({
								title: '修改成功！',
								icon: 'none'
			...
			handSubmitHigh() {
				...
				this.$refs.form.validate().then((res) => {
					...
					this.formData.helperName = helperName.join(',');
					this.formData.helper = helper.join(',');
					ClueAdd(this.formData).then((data) => {
						if (data.code == 0) {
							uni.showToast({
								title: '新增成功！',
								icon: 'none'

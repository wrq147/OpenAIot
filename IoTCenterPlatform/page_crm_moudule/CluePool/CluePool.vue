<template>
	<view id="Customer">
		<top leftIcon="icon-fanhui" :isleftBack="true" leftText="Back" backgroundColor="#161A26" title="Clue pool" rightIcon="icon-a-tianjiabantouming" class="CRM-header">
		...
		<search-compt ref="selectCompt" @openSelect="openSelect" @searching="searching" pal="Please enter the name of the clue pool"></search-compt>
		...
		methods:{
			...
			handReceive(item){
				...
				this.$refs.promptReceive.noticeOpen(
					"Are you sure to receive the customer named "+item.CompanyName+"?"
				)
				this.EditId=item.Id
			},
			handEditCull(id){

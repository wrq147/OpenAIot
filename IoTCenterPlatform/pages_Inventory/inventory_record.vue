	...
	<view>
		...
		<uni-nav-bar :status-bar="false" :fixed="true" :border="false" :height="zhanweiHei" backgroundColor="#161A26">
			<template v-slot:allslot>
				<view class="device_list_con details_top_con" id="detail_top">
					...
					<view class="date_con" style="color: #fff;">
						<uni-datetime-picker ref="dateChoice1" class="date" type="date"
							...
							<view class="date_slot" :class="{'has_val':choiceTime[0]}">
								...
								<view class="icons_con t-icon-yichu" @click.stop="clearDate1" v-if="choiceTime[0]">
								</view>
					...
					<view class="date_con">
						<uni-datetime-picker ref="dateChoice2" class="date" type="date" :clear-icon="true"
							...
							<view class="date_slot" :class="{'has_val':choiceTime[1]}">
								...
								<view class="icons_con t-icon-yichu" @click.stop="clearDate2" v-if="choiceTime[1]">
								</view>
								<custom-icons iconsName="icon-xuanzeshijian" iconsSize="36rpx"
									iconsColor="rgba(255, 255, 255, 0.5)"></custom-icons>
							</view>
							...
							<view class="li_content_con">
								<view class="li_content">
									<view class="content_title">
										<view class="name" v-if="item.FormType == 1 || item.FormType == 3">Warehousing</view>
										<view class="name" v-if="item.FormType == 0 || item.FormType == 2">Outbound</view>
										...
										<view class="box_li" style="margin-top: 0;"
											v-if="item.FormType == 1 || item.FormType == 3">(After
											change:{{item.Remnant+item.LockRemnant}}+{{item.Quantity}}={{item.Remnant+item.LockRemnant+item.Quantity}})
										</view>
										<view class="box_li" style="margin-top: 0;"
											v-if="item.FormType == 0 || item.FormType == 2">(After
											change:{{item.Remnant+item.LockRemnant}}-{{item.Quantity}}={{item.Remnant+item.LockRemnant-item.Quantity}})
										</view>
									</view>
									<view class="content_box">
										<view class="box_li">Original price: {{item.StockPrice?'￥'+item.StockPrice:0}}
										</view>
										<view class="box_li">Outbound price: {{item.Price?'￥'+item.Price:0}}</view>
										<view class="box_li">Average price after change:
		...
		methods: {
			...
			async dateChange1(val) {
				...
				if (this.choiceTime[0] && dayjs(val).isAfter(dayjs(this.choiceTime[1]))) {
					this.$refs.promptMsg.open('The start time cannot be greater than the end time', 2000)
			...
			async dateChange2(val) {
				...
				if (this.choiceTime[0] && dayjs(this.choiceTime[0]).isAfter(dayjs(val))) {
					this.$refs.promptMsg.open('The end time cannot be less than the start time', 2000)

<template>
	<view>
		<top :title="topTitle" leftText="Back" leftWidth="120rpx" rightWidth="120rpx" :isleftBack="true"
			leftIcon="icon-fanhui" backgroundColor="#161A26" rightIcon="icon-tianjia" @clickRight="wareRecordsAdd">
		</top>
		<search-compt @openSelect="openSelect" @searching="searching"
			pal="Please enter the source company or number"></search-compt>
		...
		<view class="records_list">
			<view class="records_li" v-for="(item,index) in tbList" @click="jumpPages(item)">
						...
						<view class="pro_num_price">
							<view class="text1">
								...
								<text style="margin-left: 10rpx;"
									v-if="item.List[0].Quantity!=null">×{{item.List[0].Quantity}}</text>
							</view>
				...
				<view class="li_bottom">
					...
					<view class="type_status" :class="getSatusText(item).cls" v-if="getSatusText(item)">
						{{getSatusText(item).text}}</view>
				</view>
			</view>
	...
	export default {
		data() {
			return {
				...
				tbList: [],
				status: 'loading',
				tabArr: ['All', 'Pending submission', 'Pending approval', 'Successful', 'To be returned', 'Returned'],
				current: 0,
				activeName: '',
				selectListParam: [{
					name: 'Warehousing method',
					params: 'EnterMethod',
					pal: 'Please select a warehouse method',
					value: null,
					localdata: [{
						text: 'Outbound',
						value: '0'
		...
		methods: {
			jumpPages(row) {
				...
				if (this.isSelect) {
					...
					if (row.Status == 0) {
						uni.navigateTo({
							url: '/pages_Inventory/warehouse_records_add?id=' + row.Id
						})
					} else {
						uni.navigateTo({
							url: '/pages_Inventory/warehouse_records_detail?id=' + row.Id
						})
					}
				}
			...
			wareRecordsAdd() {
				//入库记录添加
				uni.navigateTo({
					url: '/pages_Inventory/warehouse_records_add'
				})
			},
			getHouseList() {
				//获取仓库列表

<template>
	<view>
		...
		<v-tabs v-model="current" :scroll="false" :tabs="tabArr" color="rgba(255, 255, 255, 0.5)"
			...
			activeFontSize="28rpx" paddingItem="0" lineHeight="4rpx" :hasBorder="false" height="72rpx"
...
<script>
	...
	export default {
		data() {
			return {
				current: 0,
				tabArr: ['All items', 'Machines', 'Consumables'],
				topTitle: 'Inventory',
				status:'loading',
				querydata: {
					pageNum: 1,
		...
		methods: {
			...
			toStockDetail(row){
				...
				if(!this.isSelect){
					uni.navigateTo({
						url:'/pages_Inventory/inventory_record?info='+JSON.stringify(row)
					})
				}else{

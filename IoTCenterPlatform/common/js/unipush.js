	...
	getClientInfoLoop() {
		if (plus.push.getClientInfo() && plus.push.getClientInfo().clientid) {
			...
			plus.push.getClientInfoAsync(info => {
				...
				console.log(info.clientid, info, 'infoinfoinfoinfoinfoinfo');
				if (!info || !info.clientid) {
					...
					uni.setStorageSync('cid', cid);
					uniPushObj.cid = cid
					console.log("拿到了", uniPushObj.cid, cid);
					try {
						passCid(cid)
						console.log("rrrrrr");
					} catch (e) {
		...
		if (platform == "ios") { //这里是ios的方法
			...
			if (app.currentUserNotificationSettings) {
				...
				if (enabledTypes == 0) { //如果enabledTypes = 0 就是通知权限没有开启
					uni.showModal({
						title: 'prompt',
						content: 'Do you want to grant permission to open message notifications?',
						success: res => {
							if (res.confirm) {
								openTongZhi()
							} else if (res.cancel) {

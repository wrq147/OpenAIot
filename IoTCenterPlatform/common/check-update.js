...
function checkFunc() {
	return new Promise((resolve, reject) => {
		plus.runtime.getProperty(plus.runtime.appid, function(widgetInfo) {
			checkVersion(plus.runtime.version, widgetInfo.version).then(async (e) => {
				console.log(e,'app更新');
				if (e.data == null) {
					return resolve(e)
				}
...
function updateUseModal(packageInfo) {
	...
	let confirmText = !isWGT ? 'Jump' : 'Download'
			...
			uni.downloadFile({
				...
				success: res => {
					...
					plus.runtime.install(res.tempFilePath, {
						...
						uni.showModal({
							title: 'Whether to restart after successful installation？',
							success: res => {
								if (res.confirm) {
									//更新完重启app
									plus.runtime.restart();

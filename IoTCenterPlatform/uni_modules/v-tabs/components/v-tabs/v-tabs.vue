<template>
  <view class="v-tabs">
    <scroll-view
      ...
      :style="{ position: fixed ? 'fixed' : 'relative', zIndex,'padding':scrollPadding,'background-color':scrollBgColor,'width':scrollConWid }">
      <view
        ...
        this.createQueryHandler()
          ...
          .boundingClientRect(data => {
            ...
            if (this.pills) {
			  ...
			  if(this.fixedLineWid){
				  this.lineWidth=this.lineWidthProp
			  }
              this.lineLeft = scrollLeft + data.left + (data.width - data.width * this.lineScale) / 2 - this.container.left
			  if(this.fixedLineWid){
			  	this.lineLeft = scrollLeft + data.left + (data.width - this.lineWidthProp) / 2 - this.container.left
			  }
            }
          })
          .exec()

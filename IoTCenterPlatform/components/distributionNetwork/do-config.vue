				...
				try {
					...
					if (rs.data == true) {
						...
						if (this.verifycount > 40) {
							if (this.isNewDevice) {
								this.$refs.promptMsg.noticeOpen('配网失败，可能是设备连接的环境WiFi的密码错误，是否重新配网？', 'system prompt',
									true)
							} else {
								this.$emit('doConfigFun', 'fail');
							}
				...
				if (result.receivedMsg) {
					...
					if (!this.isOnline && this.curStep < 2) {
						if (result.receivedMsg.substr(0, 2) == 'OK') {
							...
							if (this.isNewDevice) {
								this.$refs.promptMsg.noticeOpen('配网失败，可能是设备连接的环境WiFi的密码错误，是否重新配网？', 'system prompt', true)
							} else {
								this.$emit('doConfigFun', 'fail');
							}
						}

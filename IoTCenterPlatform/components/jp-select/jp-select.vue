	...
	<view v-if="selectType" class="wallet_class" :class="type?'wallet_class_kq':'wallet_class_gb'">
		...
		<view class="wallet_content" :style="'height:'+ height" :class="type?'wallet_kq':'wallet_gb'">
			...
			<view class="wallet_content_tb" v-else>
				...
				<view class="tite">{{tite}}</view>
				<view class="operate1">
					<view v-if="select=='more'&&checkAll&&list" class="operate1_q" hover-class="click_hover"
						:style="'color:'+color" @tap="operate">
						{{list.length == item1.length?'empty':'Select All'}}
					</view>
					<view v-if="list" class="operate1_p" hover-class="click_hover" @tap="toSelect"
						:style="'color:'+color">
						be sure
					</view>
				</view>
			...
			<view>
				<scroll-view v-if="list" class="scroll-Y" scroll-y="true" :style="[style]"
					...
					<view v-for="(items,index) in list" :key="index" class="wallet_list"
						:class="xzindex===index?'wallet_listdj':''" @click="toChecked(items,index)">
						<view class="text" :style="[textStyle]"> {{items[name]||items}}</view>
						<view v-if="select=='radio'" style="width: 50rpx;">
							<radio style="transform: scale(0.7)" value="r2" :color="color"
								:checked="items[idKey]?items[idKey]==item1[idKey]:items==item1" />
						</view>
						<view v-else tyle="width: 50rpx;">
							<radio style="transform: scale(0.7)" value="r2" :color="color"
								:checked="getCheckedS(items)" />
						</view>
					</view>
				</scroll-view>
				<view v-else style="text-align: center;color: rgba(255, 255, 255, 0.5);line-height: 100rpx;font-size: 26rpx;">暂无数据</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		mounted() {
			...
			this.style = {
				'height': `calc( ${this.height} - 100rpx)`
			}
			this.textStyle = {
			...
			moreChecked(item) {
				if (!this.getChecked(item)) {
					...
					this.item1.forEach((el, index) => {
						if (item[this.idKey] ? item[this.idKey] == el[this.idKey] : el == item) {
				...
				if (this.item1 && this.item1.length > 0) {
					this.list.forEach((el, index) => {
						this.item1.forEach((item, index) => {
							if (item[this.idKey] ? item[this.idKey] == el[this.idKey] : el == item) {
								if (item[this.idKey] ? item[this.idKey] == itemS[this.idKey] : itemS == item) {
									c = true
								}
							}
						})
	...
	.wallet_class {
		...
		.wallet_content {
			...
			.wallet_content_tb {
				...
				align-items: center;
				position: relative;
				border-bottom: rgba(255, 255, 255, 0.2) solid 1rpx;
